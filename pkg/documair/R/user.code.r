...
# and copy them into a new not already existing files.
#DETAILS
...
#--------------------------------------------
{
  ...
  r.object9(nobjects,"character");
  r.object9(nfiles,"file");
  r.object9(file,"character",1);
  if (r.fidi9(file)=="f") {
...
# \samp{r.text3places8word} is very limited.
#DETAILS
...
#--------------------------------------------
{
  ...
  r.object9(what,"character",1,c("d","v","n","u","t","c","p"),
...
#           Checking means that the program r.pause after each display
...
#--------------------------------------------
{
  ...
  # checking arguments
  r.object9(display,"logical",1,mensaje="'display' argument not accepted");
  if (display) { r.form3title("Preparation by /prepare8pkg/",2);}
  r.object9(pkg,"character",1,mensaje="Bad package name");
  r.object9(perdir,"directory",1,mensaje="Bad 'perdir' name");
  r.object9(pkgdir,"directory",1,mensaje="Bad 'pkgdir' name");
  r.object9(signature,"integer",1,c(0,2),mensaje="Bad 'signature' argument");
  r.object9(check,"logical",1,mensaje="'check' argument not accepted");
  # normalizing the directories
  perdir <- r.dipa(perdir);
  pkgdir <- r.dipa(pkgdir);
  ...
  if (display) {
    r.form3title("Getting the available files",2);
  ...
  if (display) { r.form3title("Getting and Reading the available objects",2);}
  ...
  for (of in ofi) {
    ...
    if (display) { if (ofdif) {
      r.form3title(paste0("   Code file: ",of),1);
    }}
    ...
    bibi <- r.file2text(fifi,clean=FALSE,comment=character(0));
    anan <- code7objects4text6tags(bibi);
    if (of.typ %in% c("e","h")) {
      # non aliased situation
      for (ico in r.bf(anan)) {
        ...
        if (ico.exp) {
          ...
          if (r.fidi9(fi1rd)=="f") {
            ...
            lyse$kwd <- r.text3translate(lyse$kwd,whi$kwt);
      ...
      for (ico in r.bf(anan)) {
        ...
        if (ico==1) {
          ...
          if (r.fidi9(fi1rd)=="f") {
            ...
            while (r.void9(lyse$des[1])&(length(lyse$des)>0)) { lyse$des <- lyse$des[-1];}
            ...
            while (r.void9(lyse$exa[1])&(length(lyse$exa)>0)) { lyse$exa <- lyse$exa[-1];}
            ...
            while (r.void9(lyse$val[1])&(length(lyse$val)>0)) { lyse$val <- lyse$val[-1];}
          ...
          if (r.fidi9(fi1rd)=="f") {
            ...
            while (r.void9(llyse$des[1])&(length(llyse$des)>0)) { llyse$des <- llyse$des[-1];}
            ...
            while (r.void9(llyse$exa[1])&(length(llyse$exa)>0)) { llyse$exa <- llyse$exa[-1];}
            ...
            ## Adding the value
            while (r.void9(llyse$val[1])&(length(llyse$val)>0)) { llyse$val <- llyse$val[-1];}
  ...
  for (tf in tfi) {
    ...
    if (display) { r.form3title(paste0("   Test file: ",tf),1);}
    if (display) { cat("\n");}
  ...
  #
  if (display) {
    r.form3title(paste0(nobjt, " objects were found in total"),1);
    r.form3title(paste0(nobje, " of them will be exported to the users"),1);
    r.form3title(paste0(nobjud," of them have been documented by hand"),1);
  }
  if (nobjt == 0) {
    stop(paste0("No '*.code.r' files found into ",perdir));
  }
  # dealing with the presentation file
  if (display) { r.form3title("Getting the general presentation file",2);}
  ...
  if (!file.exists(fipres)) {
    ...
    if (signature >0 ) {
      ...
      presentation <- r.file2text(fipres);
      ...
      if (signature > 1) {
        presentation <- c(presentation,paste("       on",r.now("d")));
  ...
  if (display) { r.form3title("making the NAMESPACE",2);}
  finsp <- file.path(bpkgdir,"NAMESPACE");
  sink(finsp);
  cat("####### EXPORTED OBJECTS ###\n");
  cat("export(",exobj[1]);
  for (ii in r.bf(exobj[-1])) { cat(",\n       ",exobj[1+ii]);}
  cat(");\n");
  if (length(fsrc) > 0) {
    cat("####### EXPORTED OBJECTS ###\n");
    ...
    for (fex in fsrc) {
      ...
      fff <- paste(fess[r.bc(length(fess)-1)],sep=".");
      cat(",",fff,sep="");
    }
    cat(");\n");
  }
...
#--------------------------------------------
{
  ...
  res <- character(0);
  # checking arguments
  if (display) { r.form3title("Preparation by /compile8pkg/",2);}
  r.object9(pkg,"character",1,mensaje="Bad package name");
  r.object9(pkgdir,"directory",1,mensaje="Bad 'pkgdir' directory");
  r.object9(chkdir,"directory",1,mensaje="Bad 'chkdir' directory");
  r.object9(resdir,"directory",1,mensaje="Bad 'resdir' directory");
  r.object9( what,"character",1,mensaje="Bad 'what' option");
  r.object9(display,"logical",1,mensaje="Bad 'display' option");
  # completing the pkg directory
  pkgdir <- paste0(r.dipa(pkgdir),"/",pkg);
  # checking the package
  if (display) { r.form3title(paste0("Checking ",pkg),2);}
  system(paste0("R CMD check -o ",chkdir," ",pkgdir),intern=!display);
  # looking for the version number
  if (display) { r.form3title("Looking for the version number",2);}
  pkgdirD <- file.path(pkgdir,"DESCRIPTION");
  vv <- r.file2text(pkgdirD);
  versi <- "";
  for (ii in r.bf(vv)) {
    if (substr(vv[ii],1,8)=="Version:") {
      versi <- substr(vv[ii],9,nchar(vv[ii]));
      versi <- r.form3crop(versi," ");
    }
  }
  if (versi == "") {
    r.erreur(pkgdirD,"No version number found in that file",w=TRUE);
  }
  # specific checking for /documair/
  if (pkg=="documair") {
    if (documair0$version$v != versi) {
      r.form3display(versi);
      r.form3display(documair0$version$v);
      stop("The version constant is inconsistent for /documair/!");
    }
  }
  # Checking directory
  check7dir <- file.path(r.dipa(chkdir),paste0(pkg,paste0(".Rcheck/")));
  # copying the documentation
  if (r.belong9("p",what,lower=TRUE)) {
    if (display) { r.form3title("Copying the pdf manual",2);}
    fi1doc <- file.path(check7dir,paste0(pkg,"-manual.pdf"));
    fi2doc <- file.path(r.dipa(resdir),paste0(pkg,"-manual.pdf"));
  ...
  # making the tar.gz
  if (r.belong9("z",what,lower=TRUE)) {
    if (display) { r.form3title("Making the tar ball file",2);}
    system(paste0("R CMD build --resave-data ",pkgdir),intern=!display);
    tgzaa <- file.path(r.dipa(getwd()),paste0(pkg,"_",versi,".tar.gz"));
    tgzoo <- file.path(r.dipa(resdir),paste0(pkg,"_",versi,".tar.gz"));
  ...
  if (!r.belong9("l",what,lower=TRUE)) {
...
#--------------------------------------------
{
  ...
  res <- c(res,r.object9(display,"logical",1,mensaje="'display' must be TRUE or FALSE",fatal=FALSE));
  res <- c(res,r.object9(pkg,"character",1,mensaje="Bad package name",fatal=FALSE));
  res <- c(res,r.object9(documair7dir,"directory",1,mensaje="Bad 'documair7dir' directory",fatal=FALSE));
  ddir <- r.object9(destination7dir,"character",1,mensaje="Bad 'destination7dir' string");
  if (length(ddir)==0) {
    if (r.fidi9(destination7dir)!="d") {
      if (!dir.create(destination7dir)) {
        res <- c(res,paste("Impossible to create",destination7dir,"as a directory"));
      }
    }
  }
  res <- c(res,r.object9(what,"character",1,mensaje="Bad 'what' option",fatal=FALSE));
  res <- c(res,r.object9(signature,"integer",1,c(0,2),mensaje="Bad 'signature' argument",fatal=FALSE));
  res <- c(res,r.object9(check,"logical",1,mensaje="'check' argument not accepted",fatal=FALSE));
  if (length(res)>0) { return(res);}
  what <- tolower(what);
  ## PREPARATION
  # making the directory where to place the arborescence
  prepdir <- paste0(getwd(),"/","preparation7dir.",r.now("d"));
  if (r.fidi9(prepdir)=="d") {
