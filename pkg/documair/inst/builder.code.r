...
{
  # some checking
  r.object9(text,"character",mensaje="'code7objects4text6tags' awaits for a 'character' as first argument");
  r.object9(otags,"list",mensaje="'code7objects4text6tags' awaits for a 'list' as second argument");
  r.object9(bra,"character",2,names(otags),mensaje="'bra' not included in 'names(otags)'");
  # getting the tags values
  brack <- c(otags[[bra[1]]]$v,otags[[bra[2]]]$v);
  # getting the different pieces
  res <- r.text3brackets(text,bra=brack,
                       col1=c(1,1),col2=c(1,1),rembra=!addbra);
  # removing empty lines
  for (cc in r.bf(res)) {
    res[[cc]] <- r.text3preparation(res[[cc]],"R");
  }
  # degenerate case
...
#--------------------------------------------
{
  ...
  care <- "|||";
  # some checking
  r.object9(code,"character",mensaje="'parse8code' awaits for a 'character' as first argument");
  r.object9(otags,"list",mensaje="'parse8code' awaits for a 'list' as second argument");
  r.object9(tags,"character",-1,names(otags),mensaje="'tags' not included in 'names(otags)'");
  # getting name and usage components from the first lines
  outi <- r.text3places8word(code,otags[["tit"]]$v,column=c(1,1));
  if (dim(outi)[1] != 1) {
    print(code);
    print(outi);
    stop("The 'tit' tags was not found in the code.");
  }
  entete <- code[r.bc(outi[1,1]-1)];
  ...
  if (!is.na(gtest)) {
    ...
    usa <- paste(nom,"(",
                 paste(uuu[r.bc(length(uuu)-1)+1],collapse="("),
                 sep="");
  ...
  tagpl <- r.places4text6tags(code,tagos);
  ...
  res <- r.texts4text(code,tagpl[,c(1,2,4),drop=FALSE],
  ...
  for (tt in names(res)) {
    ...
    if (ccc == 0) {
      ...
      pte <- r.text3preparation(res[[tt]],preparation=ppp,content=ccc);
      res[[tt]] <- pte;
    }
  }
...
#--------------------------------------------
{
  # some checking
  obli <- c("name","des","tit");
  r.object9(parsed,"list",mensaje="'make8rd' awaits for a 'list' as first argument");
  if (length(unique(obli,names(parsed))) > length(names(parsed))) {
    r.form3display(parsed);
    r.erreur(obli,"At least, one of these components is missing in 'parsed'!");
  }
  # initializing
  res <- c("%",paste0("% created by /documair/ at ",r.now("dm")),"%");
  # alias
  ...
  for (ix in r.bf(stan)) {
    ...
    if ((!is.null(parsed[[vx]]))&(length(parsed[[vx]])>0)) {
      mettre <- parsed[[vx]];
      if (nx == "usage") {
        for (im in r.bf(mettre)) {
          imv <- mettre[im];
          sanom <- strsplit(imv,"(",fixed=TRUE)[[1]];
          if (length(sanom)>1) {
            # it is a fonction
            poner <- r.form3crop(sanom[1]);
