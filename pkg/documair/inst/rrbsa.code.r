...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.places4text6tags <- function (text,tags,check=FALSE)
...
# function \samp{r.text3places8word} which is called to elaborate the result.\cr
...
#  \samp{r.places4text6tags("is performed",c("perf","formed"))}
...
#EXAMPLE
# r.places4text6tags("Bonjour Monsieur","on");
# r.places4text6tags("Bonjour Monsieur",c("on","ons","mon"));
# \dontrun{r.places4text6tags("Bonjour Monsieur",c("on","ons","mon"),check=TRUE)};
# r.places4text6tags(r.rbsa0$text1$v,c("1","2","7"));
# r.places4text6tags(r.rbsa0$text2$v[1:3],"t");
# r.places4text6tags(r.rbsa0$text3$v,"uu");
#REFERENCE
#SEE ALSO
#CALLING
#COMMENT
...
#--------------------------------------------
{
  ...
  ftag <- character(0);
  for (ww in r.bf(tags)) {
    rr <- r.text3places8word(text,tags[ww]);
    if (!all(rr[,1]==rr[,3])) {
      stop("Unexpected result found in 'r.places4text6tags'");
    }
    posi <- rbind(posi,cbind(rep(ww,nrow(rr)),rr));
    fftag <- rep(names(tags[ww]),nrow(rr));
    # adding numbering when more than one occurrence
    if (length(fftag)>1) {
      fftag <- paste(r.bf(fftag),fftag,sep=".");
    }
    ftag <- c(ftag,fftag);
  }
  ...
  if (nrow(posi)>1) {
    ...
    if (check) {
      ...
      if (!all(!crit)) {
        ...
        r.erreur(NULL,"Overlapping discovered...");
      }
    }
  } else {
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.explore8list <- function(lili,monitor=r.rbsa0$monitor$v)
...
#DETAILS
# \samp{r.rbsa0$sep1$v} is used to join the names, as a consequence it must
...
#            see the \samp{r.rbsa0$monitor$v} provided object as an example.>>
#VALUE
# The resulting character matrix (see the description section).
#EXAMPLE
# r.explore8list(r.rbsa0$lis1$v);
#REFERENCE
#SEE ALSO
#CALLING
#COMMENT
...
# checking
if (monitor$chk$v) {
    r.object9(lili,"list",-1,mensaje="lili must be a list");
}
#
# starting the table
...
if (length(lili) == 0) { return(tab);
    ...
    for (ii in r.bf(lili)) {
        ...
        if (nom=="") { nom <- "<NA>";}
        if (r.belong9(r.rbsa0$sep1$v,nom)) {
            r.erreur(list(r.rbsa0$sep1$v,nom),
                   "This name for the list comprises 'r.rbsa0$sep1$v' which is not accepted by 'r.explore8list'");
        }
...
while (qq > 0) {
	...
	for (ii in r.bf(coco)) {
	    ...
	    if (is.null(nom)) { nom <- "<NA>";}
	    if (nom=="") { nom <- "<NA>";}
            noms <- paste(tab[qq,"names"],nom,sep=r.rbsa0$sep1$v);
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.get8comp7list <- function(lili,tata,
                       monitor=r.rbsa0$monitor$v)
#TITLE  returns components from a list structure
#DESCRIPTION
# This is not a end-user function at all! To understand it, one must
# have in mind how works the function \samp{r.explore8list}.\cr
# Returns components from a list structure as a one level list.
# The list \samp{lili} must have been explored with \samp{r.explore8list}
...
#{tata} << The lines of the table provided by \samp{r.explore8list}.>>
#[INPUTS]
#{monitor} <<List of constants indicating the monitoring choices,
#            see the \samp{r.rbsa0$monitor$v} provided object as an example.>>
#VALUE
...
# vv <- r.explore8list(uu);
# r.get8comp7list(uu,vv[7,])[[1]];
##
# uu <- list(1:3,
...
if (monitor$chk$v) {
    r.object9(lili,"list",-1,mensaje="lili must be a list");
    #
    r.object9(tata,"character",-1,
               mensaje="'tata' must be a 'character'");
    if (!is.matrix(tata)) {
        r.object9(tata,"character",length(nta),
                   mensaje="When not a matrix, 'tata' must have the same length that 'nta'");
    } else {
        if (ncol(tata) != length(nta)) {
            r.erreur(tata,"The 'tata' matrix must have got as many columns as the length of nta");
        }
        if (is.null(dimnames(tata)[[2]])) {
            r.erreur(tata,"The 'tata' matrix must have named columns");
        }
        if (length(union(dimnames(tata)[[2]],nta))>length(nta)) {
            r.erreur(list(tata,nta),"The 'tata' matrix must have named columns with 'nta'");
        }
    }
}
# preparation
...
names(res) <- nono;
for (ii in r.bc(nrow(tata))) {
    coco <- paste("lili[[",
                  paste(strsplit(nunu[ii],r.rbsa0$sep0$v)[[1]],
                        collapse="]][["),
                  "]]",sep="");
    coco <- paste("coco <-",coco);
    eval(parse(text=coco));
...
#DESCRIPTION
# The reverse operation of \samp{r.text2list}. The list must be a
...
# The idea is to get a character compatible with \samp{r.text2list} to produce back
...
# the \samp{r.rbsa0$sep0$v} constant but this global constant can be
# conveniently modified. The same for \samp{r.rbsa0$sep1$v}.\cr
...
#EXAMPLE
# r.list2text(r.rbsa0$lis1$v);
# r.text2list(r.list2text(r.rbsa0$lis1$v));
#REFERENCE
#SEE ALSO 
#CALLING
#COMMENT
...
if (monitor$chk$v) {
    r.object9(lili,"list",-1,mensaje="lili must be a list");
    #
    r.object9(tags,"character",c(2,Inf),mensaje="tags must be a CHARACTER matrix with two columns");
    if (!is.matrix(tags)) {r.erreur(tags,"tags must be a character MATRIX of two columns");}
    if (ncol(tags)!=2) {r.erreur(tags,"tags must be a character matrix OF TWO COLUMNS");}
    if (length(unique(tags[,1]))!= nrow(tags)) { r.erreur(tags,"Opening tags are not unique!");}
    #
    r.object9(stag,"character",3,mensaje="stag must be a character of length three");
    if (any(nchar(stag) == 0)) { r.erreur(stag,"stag must not have zero length components");}
    if (length(grep(" ",stag[c(1,3)])) > 0) { r.erreur(stag,"stag must not comprise spaces in first and/or third position");}
    #
    r.object9(comment,"character",1,mensaje="'comment' must be a character of length one");
    #
    r.object9(comments,"character",-1,mensaje="'comment' must be a character of any length");
}
# 
# preparing
res <- character(0);
...
res <- c(res,comment);
res <- c(res,paste(comment,"  This character was created on",r.now("d"),"by list2file of rbsa package"));
res <- c(res,paste(comment,"  from the object :",deparse(substitute(x))));
res <- c(res,comment);
res <- c(res,comment);
for (ii in r.bf(comments)) {
    res <- c(res,paste(comment,comment,r.rbsa0$sep0$v,r.rbsa0$sep0$v,comments[ii]));
}
res <- c(res,comment);
res <- c(res,comment);
#
...
if (length(lili) == 0) {
    ...
    # exploring the list
    liliex <- r.explore8list(lili);
    # checking the list-vma nature 
    if(!all(liliex[,"classes"] %in% c("list",
                                      "logical","integer","numeric","character",
                                      "matrix","array"))) {
        r.erreur(liliex,"The 'lili' list is not composed of list/vector/matrix/array");
    }
    # checking the existence of names for all levels
    if (any(is.na(liliex[,"name"]))) {
        r.erreur(liliex,"Not all 'lili' components have got a name");
    }
    # checking the levels
    xlev <- max(as.numeric(liliex[,"depth"]));
    if (xlev > nrow(tags)) {
        r.erreur(list(liliex,tags),"'lili' has got too many levels for the proposed 'tags'");
    }
    #
    # ordering the table
    ...
    for (ii in uv) {
        ...
        if (niv == 1) { res <- c(res,paste0(comment,r.form3repeat("=",20)));}
        if (niv == 2) { res <- c(res,comment);}
        rrr <- paste0(tags[niv,1],liliex[ii,"name"],tags[niv,2]);
        if (liliex[ii,"classes"] != "list") {
            coco <- r.get8comp7list(lili,liliex[ii,],monitor=monitor)[[1]];
            caca <- r.vma2text(coco,r.rbsa0$sep1$v);
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.vma2text <- function(x,xsep=r.rbsa0$sep1$v,
                       monitor=r.rbsa0$monitor$v)
...
# inverse function of \samp{r.text2vma}. This vector is the first
...
# \samp{[[2]]} the type according to \samp{r.text2vma}.
#EXAMPLE
#####
## vectors
# r.vma2text(letters);
...
# xx <- r.vma2text(x);
# r.text2vma(xx[[1]],xx[[2]]);
# r.vma2text(character(0));
#####
## matrices
# x <- matrix(1:20,4);
# r.vma2text(x);
...
if (monitor$chk$v) {
    #
    r.object9(xsep,"character",1,mensaje="must indicate the character string of separation");
    #
    if (!is.vector(x) &
        !is.matrix(x) &
        !is.array(x)) {
        r.erreur(class(x),"'x' must be a vector or a matrix or an array!");
    }
}
#
res <- vector("list",2);
...
if (is.array(x)&(!is.matrix(x))) {
    ...
    if (is.null(dimnames(x))) {
        res[[2]] <- r.rbsa0$vma$v["a"];
    } else {
        nna <- dimnames(x);
        for (hh in r.bf(nna)) {
            if (is.null(nna[[hh]])) {
                nna[[hh]] <- r.bc(dim(x)[hh]);
            }
        }
...
# definitions and names can be obtained with the function \samp{r.display8k}.\cr
# An important role of the constant is to drive the behavior of many functions
# through \samp{r.rbsa0$monitor$v} which is itself a \samp{rbsa} constant.
#DETAILS
...
#EXAMPLE
# print(r.display8k(r.rbsa0,"names"));
# print(r.display8k(r.rbsa0));
# print(r.display8k(r.rbsa0$monitor$v,"names"));
# print(r.display8k(r.rbsa0$monitor$v));
# str(r.rbsa0$monitor$v);
#REFERENCE
#SEE ALSO r.rbsa7list9 r.display8k
#CALLING
#COMMENT
#FUTURE
#AUTHOR J.-B. Denis
...
#--------------------------------------------
{
  ...
  list(
       ...
       #
       vma=list(v=vma,
                d="Different types of vma (see r.text2vma for details)"),
       sep0=list(v=" ",
       ...
       monitor=list(v=list(chk=list(v=TRUE, d="must arguments be checked?"),
                           pau=list(v=FALSE, d="must call to r.pause be effective in some functions?"),
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.text3places8word <- function(text,word,
                            column=c(1,Inf),
                            which=c(1,Inf)
                            )
...
# the associated intervals as defined in the function \samp{r.text3interval}. 
...
#EXAMPLE
# r.text3places8word(letters,"j");
# r.text3places8word(letters,"J");
# r.text3places8word(c("Il etait une fois","un petit et rouge chaperon"),"et");
# r.text3places8word(c("Il etait une fois","un petit et rouge chaperon"),"et",which=c(2,3));
# r.text3places8word(c("Il etait une fois","un petit et rouge chaperon"),"et",which=-c(1,3));
# r.text3places8word(c("# Il etait une fois"," #un petit et rouge chaperon"),"#");
# r.text3places8word(c("# Il etait une fois"," #un petit et rouge chaperon"),"#",column=c(1,2));
# r.text3places8word(c("# Il etait une fois"," #un petit et rouge chaperon"),"#",column=c(2,2));
# # overlapping pattern  
# r.text3places8word("aaaa","aa");
#REFERENCE
#SEE ALSO 
#CALLING
#COMMENT
...
{
  # checking
  r.object9(text,"character");
  r.object9(word,"character",len=1);
  r.object9(which,"numeric",len=2);
  r.object9(column,"numeric",len=2);
  # initializing
  ...
  for (nbl in r.bf(text)) {
    # looking in the line
    if (ouhou[[nbl]][1]>0) {
      # at least an occurrence
      for (oo in r.bc(length(ouhou[[nbl]]))) {
        nbw <- nbw + 1;
        if (nbw >= which[1]) {
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.object9 <- function(x,typ,len=-1,con=numeric(0),
...
# resulting TRUE with function \samp{r.void9}.
#KEYWORDS error
#INPUTS
...
#EXAMPLE
# r.object9(letters,c("numeric","character"),c(20,30),mensaje="!OK");
# r.object9(structure("A",.Names="a"),"ncharacter",-1,LETTERS);
# r.object9("A","ncharacter",-1,mensaje="BAD",fatal=FALSE);
# r.object9(structure("A",.Names="a"),"character",-1,letters,"bad",fatal=FALSE);
#REFERENCE
#SEE ALSO
#CALLING
#COMMENT
...
if (is.null(x)) {
	...
	cat("From r.object9: in fact 'x' is '",
	    deparse(substitute(x)),"'\n");
	print.default(x);
	if (fatal) {r.erreur("Message:",paste(res));}
    }
}
if (all(!is.na(match(typ,typena)))) { if (any(is.na(x))) { 
    r.erreur(typ,"OBJECT9: 'x' is or comprises 'NA' which were not removed!");
}}
# possible type for the check
type <- types[type[!is.na(type)]];
if (!is.numeric(len)) {
    r.erreur(len,"OBJECT9: 'len' must be NUMERIC of size one or two");
}
...
if (!("any" %in% type)) {
    ...
    for (tt in type) {
        ...
        if (tt=="null")      { if (r.void9(x))        { ty <- TRUE;}}
        #
        if (tt=="file") {
          if (all(r.fidi9(x) == "f")) { ty= TRUE;
          } else { res <- "Not all components were detected as file(s)";}
        }
        if (tt=="directory") {
          if (all(r.fidi9(x) == "d")) { ty= TRUE;
...
if (!is.null(speci)) {
  ...
  if (is.vector(x)) {
    if (speci=="named") {
      ...
      r.erreur(speci,"For vectors, the only valid value for 'speci' is 'named'");
    }
  }
  # for matrices
  if (is.matrix(x)) {
    if (is.matrix(speci)) {
      if (all(dim(speci)==2)) {
        if (!any(is.na(speci[1,]))) {
          if (abs(r.interv7belonging(nrow(x),speci[1,]))>1) {
        ...
        if (!any(is.na(speci[2,]))) {
          if (abs(r.interv7belonging(ncol(x),speci[2,]))>1) {
            inter <- paste(speci[2,],collapse="-");
            res <- c(res,paste("The number of columns is not in the specified",inter,"interval"));
          }
        }
      } else {
        r.erreur(speci,"For matrices, the only valid value for speci is a matrix of dimension '2x2'");
      }
    } else {
      r.erreur(speci,"For matrices, the only valid value for speci is a matrix of dimension '2x2'");
    }
  ...
  if (is.array(x) & !(is.matrix(x))) {
    if (is.matrix(speci)) {
      if (ncol(speci)==2) {
        if (!any(is.na(speci[1,]))) {
          if (abs(r.interv7belonging(length(dim(x)),speci[1,]))>1) {
            inter <- paste(speci[1,],collapse="-");
            res <- c(res,paste("The number of dimensions is not in the specified",inter,"interval"));
          }
        }
        for (dd in r.bf(dim(x))) { if (nrow(speci)>dd) {
          if (!any(is.na(speci[dd+1,]))) {
            if (abs(r.interv7belonging(dim(x)[dd],speci[dd+1,]))>1) {
        ...
        r.erreur(speci,"For arrays, the only valid value for speci is a matrix with '2' columns");
      }
    } else {
      r.erreur(speci,"For arrays, the only valid value for speci is a matrix with '2' columns");
    }
...
if (!identical(res,character(0))) {
	...
	print.default(x);
        print(res);
        r.erreur(NULL,"FATAL ERROR from r.object9");
    }
}
res;
}
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.form3crop <- function(chaine,bef=r.rbsa0$sep0$v,aft=bef,
                      mxm=Inf,empty=FALSE,
                      monitor=r.rbsa0$monitor$v)
...
#EXAMPLE
# r.form3crop('IMPORTANT','IM',' ANT');
# r.form3crop(c('   OUF ',' FOU ',''),' ','',1);
#REFERENCE
#SEE ALSO
#CALLING
#COMMENT
...
# checking
if (monitor$chk$v) {
    r.object9(chaine,"character",-1,mensaje="r.form3crop: 'chaine' must be a character");
    r.object9(bef,"character",1,mensaje="r.form3crop: Vector are not accepted for 'bef'");
    r.object9(aft,"character",1,mensaje="r.form3crop: Vector are not accepted for 'aft'");
    r.object9(mxm,"numeric",1,mensaje="r.form3crop: mxm must be numeric(1)");
}
# null case
if (length(chaine) == 0) { return(chaine);}
lb <- nchar(bef);
la <- nchar(aft);
for (ich in r.bf(chaine)) {
    cha <- chaine[ich];
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.file2text <- function(file,path="",
                      clean=TRUE,
                      ended=")STOP(",
                      comment="#",
...
# checking
if (monitor$chk$v) {
    r.object9(file,"character",1,mensaje="file must indicate the name of one file");
    #
    r.object9(path,"character",1,mensaje="path must indicate the name of one directory");
    #
    if (path!="") { fifi <- paste(path,file,sep="/");} else { fifi <- file;}
    if (is.na(file.info(fifi)$size)) { r.erreur(fifi,"This file seems not to exist.");}
    #
    r.object9(clean,"logical",1,mensaje="clean must be a logical(1)");
    #
    r.object9(ended,"character",1,mensaje="ended must be a character of length one");
    #
    r.object9(comment,"character",-1,mensaje="comment must be a character");
    #
    r.object9(skip,"character",c(2,Inf),mensaje="skip must be a CHARACTER matrix with two columns");
    if (!is.matrix(skip)) {r.erreur(skip,"skip must be a character MATRIX of two columns");}
    if (ncol(skip)!=2) {r.erreur(skip,"skip must be a character matrix OF TWO COLUMNS");}
    #
    r.object9(include,"character",1,mensaje="include must be a character of length one");
    #
}
#
...
#
# cleaning
if (clean) { for (ii in r.bf(lu)) {
    # removing framing spaces
    lu[ii] <- r.form3crop(lu[ii]," "," ");
}}
#
...
if (!r.void9(comment)) {
  mu <- r.filter8text(lu,comment,remove=TRUE,
                         exact=8,lower=FALSE,
                         monitor=monitor);
}
#
# removing the skipped lines
for (nn in r.bc(nrow(skip))) {
    deb <- skip[nn,1]; fin <- skip[nn,2];
    debu <- substr(lu,1,nchar(deb)) == deb;
    fini <- substr(lu,1,nchar(fin)) == fin;
    sk1 <- apply(outer(which(debu)  ,r.bf(lu),"<="),2,sum);
    sk2 <- apply(outer(which(fini)+1,r.bf(lu),"<="),2,sum);
    nsk <- ((sk1-sk2) < 1);
    lu <- lu[nsk];
}
#
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.fidi9 <- function(chemins,retour=c("-","f","d"))
...
#EXAMPLE
# r.fidi9(c("/","~","~/.bashrc",getwd(),"toto.txt"));
#REFERENCE
#SEE ALSO r.dipa
#CALLING
#COMMENT
...
#--------------------------------------------
{
  ...
  r.object9(chemins,"character");
  r.object9(retour,"character",3);
  # initializing
  res <- rep(retour[1],length(chemins));
  # proceeding
  for (ii in r.bf(chemins)) {
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.now <- function(what="dh",format="red",seps=c("_","@",":"))
...
{
# checking
r.object9(seps,"character",c(1,Inf));
if (length(seps) < 3) { seps <- c(seps,seps,seps);}
# adding the consequences to what
if (r.belong9("s",what)) { what <- paste0(what,"m");}
if (r.belong9("m",what)) { what <- paste0(what,"h");}
# getting the moment
mnt <- Sys.time();
...
if (format[1]=="red") {
    if (r.belong9("d",what)) {
        ...
        if (r.belong9("h",what)) { res <- paste0(res,seps[2]);}
    }
    if (r.belong9("h",what)) {
        res <- paste0(res,heure);
        if (r.belong9("m",what)) { res <- paste0(res,seps[3]);}
    }
    if (r.belong9("m",what)) {
        res <- paste0(res,minute);
        if (r.belong9("s",what)) { res <- paste0(res,seps[3]);}
    }
    if (r.belong9("s",what)) {
    ...
    if (r.belong9("d",what)) {
        ...
        if (r.belong9("h",what)) { res <- paste0(res," `a ");}
    }
    if (r.belong9("h",what)) {
        res <- paste0(res,heure,"H");
        if (r.belong9("m",what)) { res <- paste0(res,"");}
    }
    if (r.belong9("m",what)) {
        res <- paste0(res,minute,"m");
        if (r.belong9("s",what)) { res <- paste0(res,"");}
    }
    if (r.belong9("s",what)) {
...
#CALLING
#COMMENT
# r.bf for Boucle For the elements of an object
#FUTURE
#AUTHOR J.-B. Denis
#CREATED 08_09_05
#REVISED 08_09_05
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.belong9 <- function(sch,ch,exact=FALSE,how="a",lower=FALSE,
                       monitor=r.rbsa0$monitor$v)
...
#EXAMPLE
# r.belong9('a','non');
# r.belong9('o',c('non','oui'));
# r.belong9('o',c('non','oui'),6);
# r.belong9('o',c('non','oui'),8);
# r.belong9('O',c("oui"));
# r.belong9('O',c("oui"),lower=TRUE);
# r.belong9(c('o','n'),c('non','oui'),6,how='v');
# r.belong9(c('o','n'),c('non','oui'),8,how='v');
# r.belong9(c("o","oui"),c('non','oui'));
# r.belong9(c("o","oui"),c('non','oui'),how="v");
# r.belong9(c("A[SEX]","A[AGE]"),c("A[AGE]","A[SEX]"),how="a")
#REFERENCE
#SEE ALSO
#CALLING
#COMMENT
...
#--------------------------------------------
{
  ...
  #
  if (monitor$chk$v) {
      r.object9(  sch,"character",c(1,Inf));
      r.object9(   ch,"character",c(1,Inf));
      if (is.logical(exact)) {exact <- exact*10;}
      r.object9(as.character(exact),"character",1,c(0,6,8,10));
      r.object9(  how,"character",1);
      # because 'r.object9' calls 'r.belong9' for this type of check!
      if (!(how %in% c("v","1","a"))) {
          r.erreur(how,"Not accepted value for 'how'");
      }
    r.object9(lower,"logical",1);
  }
  if (lower) {
    sch <- tolower(sch);
    ch <- tolower(ch);
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.text3replace <- function(text,translate,lowering=FALSE,
...
# r.text3replace(letters,tra);
# r.text3replace(c("Il etait une fois","un petit et rouge chaperon"),tra);
# r.text3replace(c("Il etait une fois","un petit et rouge chaperon"),tra,b.accepted=" ");
#REFERENCE
#SEE ALSO r.text3acceptance
#CALLING
...
#--------------------------------------------
{
  ...
  r.object9(translate,"character");
  if (length(translate)==2) { translate <- matrix(translate,1);}
  r.object9(translate,"matrix",speci=matrix(c(NA,2,NA,2),2));
  r.object9(b.accepted,c("null","character"));
  r.object9(b.rejected,c("null","character"));
  r.object9(a.accepted,c("null","character"));
  r.object9(a.rejected,c("null","character"));
  if ((!is.null(b.accepted)) & (!is.null(b.rejected))) {
    r.erreur(list(b.accepted,b.rejected),"'b.accepted' and 'b.rejected' cannot be active simultaneously");
  }
  if ((!is.null(a.accepted)) & (!is.null(a.rejected))) {
    r.erreur(list(a.accepted,a.rejected),"'a.accepted' and 'a.rejected' cannot be active simultaneously");
  }
  r.object9(b.line,"logical");
  r.object9(e.line,"logical");
  # looping on each word
  for (nn in r.bc(nrow(translate))) {
    amo <- translate[nn,1];
    nmo <- translate[nn,2];
    # looking for the presence of the word to translate
    if (lowering) {
      ouou <- r.text3places8word(tolower(text),amo);
    } else {
      ouou <- r.text3places8word(text,amo);
    }
    # dealing with each occurrence
    for (oo in rev(r.bc(nrow(ouou)))) {
      lign <- text[ouou[oo,1]];
      # determining the acceptance
      acce <- r.text3acceptance(lign,c(ouou[oo,2],ouou[oo,4]),
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.text3brackets <- function(text,bra=c("{","}"),
...
# of the result provided by \samp{r.text3places8word}.\cr
...
#EXAMPLE
# r.text3brackets(paste(letters,collapse=""),c("j","u"));
# r.text3brackets(c(" juste {un","deux ou trois} suffiront !"));
#REFERENCE
#SEE ALSO
#CALLING r.text3places8brackets
#COMMENT
...
#--------------------------------------------
{
  # checking
  r.object9(text,"character");
  r.object9(bra,"character",len=2);
  r.object9(which,"numeric",len=2);
  r.object9(col1,"numeric",len=2);
  r.object9(col2,"numeric",len=2);
  # looking for bracket places
  uuu <- r.text3places8brackets(text,bra,col1,col2,which);
  # getting the different components
  res <- vector("list",0);
  for (con in r.bc(dim(uuu)[3])) {
    # getting the intervals
    res[[con]] <- r.text3stext(text,as.numeric((uuu[,,con])));
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.text3preparation <- function(text,preparation="rbR",
...
#EXAMPLE
# r.text3preparation(r.rbsa0$text2$v[1:3],preparation="j",llength=10)
# uu <- c("Il etait une fois un petit et rouge chaperon",
#         "qui voulait aller voir sa mere-grand");
# r.text3preparation(uu,"j",llength=20);
# r.text3preparation(uu,"j",llength=80);
# r.text3preparation( c(" Je veux   voir  "," et re-voir  "),"rbBc")
# r.text3preparation(c("# Je veux   voir  "," et re-voir  "),"rbBc")
#REFERENCE
#SEE ALSO
#CALLING
...
#--------------------------------------------
{
  ...
  r.object9(content,"numeric",len=2,con=c(1,length(text)),
               mensaje="Bad 'content' argument (2)");
  #
  r.object9(preparation,
             "character",len=1,
  ...
  if (r.belong9("w",preparation)) {
    if (length(text)== 0) {
      r.erreur(NULL,"No line found to get a first word");
    }
    tex <- text[1];
  ...
  ## concatenation
  if (r.belong9("c",preparation)|
      r.belong9("v",preparation)|
      r.belong9("V",preparation)|
      r.belong9("S",preparation)|
      r.belong9("j",preparation)) {
    res <- paste(res,collapse=" ");
  }
  ## vectorisation
  if (r.belong9("v",preparation)|
      r.belong9("V",preparation)|
      r.belong9("S",preparation)|
      r.belong9("j",preparation)) {
  ...
  ## sorting
  if (r.belong9("S",preparation)|
      r.belong9("j",preparation)) {
    res <- sort(res);
  }
  ## justification
  if (r.belong9("j",preparation)) {
    if (length(res)>0) {
      rrr <- res[-1]; res <- res[1];
      for (cc in r.bf(rrr)) {
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.dipa <- function(chemins,sep=.Platform$file.sep)
...
#  is performed, for that the function \samp{r.fidi9(r.dipa(...))} could be used.
#DETAILS
...
#EXAMPLE
# r.dipa(c("/","~","~/.bashrc",getwd(),"toto.txt"));
#REFERENCE
#SEE ALSO r.fidi9
#CALLING
#COMMENT
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.erreur <- function(x,...,w=FALSE,monitor=r.rbsa0$monitor$v)
...
#KEYWORDS error
#INPUTS
#{x} <<object to be printed before the message. When \samp{r.void9(x)}
...
#{monitor} << List of monitoring constants, see \samp{r.rbsa0$monitor} to
#             know its structure.>>
#VALUE
# nothing is returned
#EXAMPLE
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.text3places8brackets <- function(text,bra=c("{","}"),
                                 col1=c(1,Inf),col2=c(1,Inf),
                                 which=c(1,Inf))
...
# interval associated to the bracket (see function \samp{r.text3interval} for
...
#EXAMPLE
# r.text3places8brackets(paste(letters,collapse=""),c("j","u"));
# r.text3places8brackets(paste(letters,collapse=""),c("ab","xyz"));
# r.text3places8brackets(c(" juste {un","deux ou trois} suffiront !"));
#REFERENCE
#SEE ALSO
#CALLING
#COMMENT
...
#--------------------------------------------
{
  ...
  ooo <- r.text3places8word(text,bra[1],which=which,column=col1);
  # looking for closing positions
  ccc <- r.text3places8word(text,bra[2],which=which,column=col2);
  # translating them into absolute rankings
  ooa <- matrix(NA,nrow(ooo),2);
  cca <- matrix(NA,nrow(ccc),2);
  for (ii in r.bc(nrow(ooo))) {
    ooa[ii,] <- c(r.text3ij2n(ooo[ii,1:2],text),
                  r.text3ij2n(ooo[ii,3:4],text));
  }
  for (ii in r.bc(nrow(ccc))) {
    cca[ii,] <- c(r.text3ij2n(ccc[ii,1:2],text),
                  r.text3ij2n(ccc[ii,3:4],text));
  }
  # initializing
  ...
  while(nrow(ooa)>=1) {
    ...
    fipo <- 1 + r.text3ij2n(ooo[1,3:4],text);
    bons <- which(cca[,1] >= fipo);
    cca <- cca[bons,,drop=FALSE];
    if (nrow(cca) == 0) {
      print(text);
      stop("'r.text3places8brackets' found an opening bracket without a closing one!");
    }
    ccc <- ccc[bons,,drop=FALSE];
    res[,2,nbb] <- ccc[1,3:4];
    # removing the closed opening brackets
    fipo <- 1 + r.text3ij2n(ccc[1,3:4],text);
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.void9 <- function(x,vector=FALSE) 
...
#  Notice that \samp{r.void9("")} is \samp{TRUE}.
#DETAILS
...
#EXAMPLE
# \dontrun{r.void9(toto);}
# r.void9(numeric(0));
# r.void9(NULL);
# r.void9("");
# r.void9(c("","A"));
# r.void9(c("","A"),vector=TRUE);
#REFERENCE
#SEE ALSO
#CALLING
#COMMENT
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.form3title <- function(tit,empha=3,indent=2+2*empha,imp=TRUE)
#TITLE  prints or prepares a title
#DESCRIPTION
# prints or prepares the character string \samp{tit}
# with more or less emphasis.
# This function is a shortcut of the hidden function \samp{r.form3titre},
...
#EXAMPLE
# for (ii in 0:8) {r.form3title("Some Title",ii,imp=TRUE)};
#REFERENCE
#SEE ALSO
#CALLING {r.form3repeat}
#COMMENT
#FUTURE
#AUTHOR J.-B. Denis
#CREATED 07_09_24
...
# preparing the argument for r.form3titre
...
# calling r.form3titre
res <- r.form3titre(tit,box=box,
                  lbef=lbef,sbef=sbef,
                  saft=saft,laft=laft,
                  charbox=c("+","|","+",
                            "-","+","|",
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.parse8text <- function(text,item1=c("{","}"),item2=c("<<",">>"),
                       numb="#",bull="*",lsep="-")
...
#--------------------------------------------
{
  # checking
  r.object9(text,"character");
  r.object9(item1,"character",len=2);
  if (any(item1=="")) {
    r.erreur(item1,mensaje="(1) A closing cannot be ''");
  }
  r.object9(item2,"character",len=2);
  if (any(item2=="")) {
    r.erreur(item2,mensaje="(2) A closing cannot be ''");
  }
  r.object9(numb,"character",len=1);
  r.object9(bull,"character",len=1);
  r.object9(lsep,"character",len=1);
  # initialization
  ...
  b1 <- r.text3places8word(text,item1[1],column=c(1,1));
  if (nrow(b1)>0) {
    # possibly some lists are present
    eli <- TRUE;
    nb1 <- r.text3ij2n(b1[,1:2],text);
    b2 <- r.text3places8word(text,item1[2]);
    if (nrow(b2)>0) {
      nb2 <- r.text3ij2n(b2[,3:4],text);
      b3 <- r.text3places8word(text,item2[1]);
      if (nrow(b3)>0) {
        nb3 <- r.text3ij2n(b3[,1:2],text);
        b4 <- r.text3places8word(text,item2[2]);
          if (nrow(b4)>0) {
            nb4 <- r.text3ij2n(b4[,3:4],text);
  ...
  if (eli) {
    ...
    pbra <- array(NA,dim=c(4,2,nrow(bra)),
                  dimnames=list(c("s1","e1","s2","e2"),
                                c("row","col"),r.bc(nrow(bra))));
    pbra["s1",,] <- t(r.text3n2ij(bra[,1],text));
    pbra["e1",,] <- t(r.text3n2ij(bra[,2],text));
    pbra["s2",,] <- t(r.text3n2ij(bra[,3],text));
    pbra["e2",,] <- t(r.text3n2ij(bra[,4],text));
    #
    for (li in r.bc(dim(pbra)[3])) {
    ...
    if (any(egal)) {
      ...
      r.erreur(pbra[,,egal,drop=FALSE],"Some item name not on the same line");
    }
  }
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.file2list <- function(file,path="",
...
# It is just the linking of the two functions \samp{r.file2text}
# and \samp{r.text2list}, see their comments for the description of the arguments.
#DETAILS
#KEYWORDS IO
#INPUTS
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.text3translate <- function(words,translation,
...
# r.text3translate(letters,tra);
# r.text3translate(letters,tra,FALSE);
# r.text3translate(letters,tra,FALSE,FALSE);
# r.text3translate(r.rbsa0$text3$v,c(The="xxx",the="xxx"));
#REFERENCE
#SEE ALSO 
#CALLING
#COMMENT
...
{
  # checking
  r.object9(words,"character");
  r.object9(translation,"character");
  r.object9(left,"logical",1);
  r.object9(eline,"logical",1);
  # standardizing the words to translate
  words <- r.text3preparation(words,"bBR");
  # degenerate cases
  ...
  if (length(unique(names(translation)))!=length(translation)) {
    r.erreur(translation,"Some words have different translations");
  }
  # translating
  for (ww in r.bf(words)) {
    www <- words[ww];
    www <- r.text3preparation(www,"V");
    for (vv in r.bf(www)) {
  ...
  if (eline) {
    words <- r.text3preparation(words,"R");
  }
  # returning
  words;
}
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.form3titre <- function(tit,box="un",
                           lbef=0,sbef=5,
                           saft=0,laft=1,
                           charbox=c("+","|","+","-",
                                     "+","|","+","-",
...
#{alig} <<The aligment to be done by \samp{r.form3justify}: 
...
# a character string 
#EXAMPLE
# r.form3titre("Some Title");
# r.form3titre(c("The title","can comprise","several lines"),
#            box="box",lbef=4,laft=2);
# r.form3titre(c("And the box","can be incomplete", "as well!"),
#            box="cor");
# r.form3titre(c("The title","can comprise","several lines"),
#            box="box",lbef=4,laft=2,
#            charbox=c("*","+","/","=","*","+","/","="," "));
...
# checking
if (monitor$chk$v) {
    r.object9(caret,"logical",1);
    r.object9(tit,"character",-1);
    r.object9(box,"character",1);
    if (!r.belong9(box,boxes,TRUE)) {
        r.erreur(list(box,boxes),"'box' must be one of 'boxes'");
    }
    r.object9(lbef,c("numeric","character"),c(1,Inf));
    r.object9(laft,c("numeric","character"),c(1,Inf));
    r.object9(sbef,c("numeric","character"),1);
    r.object9(lbef,c("numeric","character"),1);
    r.object9(charbox,"character",9);
    if (!all(nchar(charbox)==1)) {
        r.erreur(charbox,"For the moment only unicharacter in 'charbox'");
    }
    r.object9(imp,"logical",1);
}
# the null case
if (r.void9(tit)) { return(character(0));}
# preparing
if (is.numeric(lbef)) { lbef <- rep(" ",max(0,round(lbef[1])));}
if (is.numeric(laft)) { laft <- rep(" ",max(0,round(laft[1])));}
if (is.numeric(sbef)) { sbef <- r.form3repeat(" ",max(0,round(sbef[1])));}
if (is.numeric(saft)) { saft <- r.form3repeat(" ",max(0,round(saft[1])));}
#
...
tit <- r.form3justify(tit,lmax,format=alig,carac=charbox[9]);
# producing
res <- character(0);
# first lines
for (ii in r.bf(lbef)) { res <- c(res,lbef[ii]);}
# overline
...
# last lines
for (ii in r.bf(laft)) { res <- c(res,laft[ii]);}
# adding carriage returns
if (caret) { for (ii in r.bf(res)) {
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.form3display <- function(x,pau=FALSE,cat=FALSE,...)
#TITLE  displays with its name any object
#DESCRIPTION
#  displays any object after giving the name of
# the variable containing it. A r.pause can be introduced
...
#[INPUTS]
#{pau} << Must a r.pause be performed after the display?>>
...
# the function \samp{r.file2text}, this explains why it is convenient
...
# when starting a new list, a character vma (see \samp{r.text2vma}
...
#            "<<C>> 1 2 3");
# r.text2list(text);
# r.list2text(r.text2list(text));
#REFERENCE
#SEE ALSO r.text2file r.file2list
#CALLING
#COMMENT
...
# checking
if (monitor$chk$v) {
    r.object9(text,"character",-1,mensaje="text must be a character");
    #
    r.object9(tags,"character",c(2,Inf),mensaje="tags must be a CHARACTER matrix with two columns");
    if (!is.matrix(tags)) {r.erreur(tags,"tags must be a character MATRIX of two columns");}
    if (ncol(tags)!=2) {r.erreur(tags,"tags must be a character matrix OF TWO COLUMNS");}
    if (length(unique(tags[,1]))!= nrow(tags)) { r.erreur(tags,"Opening tags are not unique!");}
    #
    r.object9(sep,"character",1,mensaje="sep must indicate one string character");
    if (nchar(sep) == 0) { r.erreur(sep,"sep must not be an empty string");}
    #
    r.object9(rsep,"character",1,mensaje="rsep must be a character(1)");
    #
    r.object9(stag,"character",2,mensaje="stag must be a character of length two");
    if (any(nchar(stag) == 0)) { r.erreur(stag,"stag must not have zero length components");}
    if (length(grep(" ",stag)) > 0) { r.erreur(stag,"stag must not comprise spaces");}
    #
    r.object9(comment,"character",1,mensaje="'comment' must be a character of length one");
}
#
...
if (!r.void9(comment)) {
  text <- r.filter8text(text,comment,remove=TRUE,
                         exact=8,lower=FALSE,
                         monitor=monitor);
}
#
...
#          and \samp{r.rbsa0$vma$v["c","v","V","w","m",...]} one of the type of vma character
#          is expected. 
nbtag <- 0;
for (i in r.bf(text)) {
    lev <- 0;
    # looking for a tag
    for (j in r.bc(nrow(tags))) { if (lev == 0) {
        pot <- r.form3crop(text[i],tags[j,1],"");
    ...
    if (lev > 0) {
        ...
        if (nchar(put[1]) < nchar(pot)) {
            ...
            if (fch %in% c(""," ",NA)) {
                ...
                tta <- which(pyt==r.rbsa0$vma$v);
                if (length(tta)==0) {
                    # default type
                    typing <- r.rbsa0$vma$v["v"];
                } else {
                    # explicite type
                    typing <- names(r.rbsa0$vma$v)[tta];
                }
            }
        tagged <- rbind(tagged,c(i,lev,put[1],"???",ssep,typing));
        } # end of nchar(put[1]) < nchar(pot)
    } # end of lev > 0
} # end of i in r.bf(text)
#
#
# in case of no list
...
for (ii in r.bc(nrow(tagged)-1)) {
    ...
    lev2 <- as.numeric(tagged[ii+1,"lev"]);
    if ((lev2-lev1) > 1) {
        r.erreur(tagged,"The progression of levels is not accepted: all levels must be introduced");
    }
}
# checking that no leaf tags are followed by another tag
for (ii in r.bc(nrow(tagged)-1)) { if (tagged[ii,"leaf"]=="no") {
    actuel <- as.numeric(tagged[  ii,"num"]);
    suivan <- as.numeric(tagged[ii+1,"num"]);
    if ((suivan-actuel) != 1) {
        r.erreur(list(tagged,ii),"In line 'ii', a 'no-leaf' tags is not immediately followed by another tag (only terminal levels can have contents)");
    }
}}
#
if (tagged[1,"lev"] != "1") {
    r.erreur(tagged,"The first level must be 1");
}
# replacing the introduced NA
for (ii in r.bf(text)) { if (is.na(text[ii])) { text[ii] <- "";}}
#
#
...
for (ll in r.bc(nrow(tagged))) {
	...
	if (rsep=="no_action") {
        ...
        chacha <- r.text2vma(choc,tagged[ll,"type"],r.rbsa0$sep1$v,monitor=monitor);
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.texts4text <- function(text,where,
...
# output of \samp{r.places4text6tags}. Empty lines can be removed.\cr
...
# r.texts4text(c(uu),ww[1,,drop=FALSE]);
# r.texts4text(c(uu),c(1,7,6));
# r.texts4text(c(uu),ww[1:2,]);
# r.texts4text(c(uu,UU),ww);
# r.texts4text(c(uu,"",UU),ww);
# r.texts4text(c(uu,"",UU),ww,rmeli=FALSE);
# r.texts4text(c(uu),ww);
# r.texts4text(c(uu),ww[1,,drop=FALSE],addbeg=FALSE);
# r.texts4text(c(uu),ww[1,,drop=FALSE],addbeg=FALSE,addend=FALSE);
#REFERENCE
#SEE ALSO
#CALLING
#COMMENT
...
#--------------------------------------------
{
  ...
  # checking
  if (length(r.object9(where,c("matrix","data.frame"))) > 0) {
    r.erreur(where,"'where' must be a matix!");
  }
  if ((ncol(where) < 2) | (ncol(where)>3)) {
    r.erreur(where,"'where' must have two or three columns");
  }
  if (nrow(where) < 1) {
    r.erreur(where,"'where' must have at least one row");
  }
  ...
  if (any(where[,3] < where[,2]-1)) {
    r.erreur(where,"Third column cannot be less that the second one!");
  }
  # dimnaming if necessary
  if (is.null(dimnames(where))) {
    dimnames(where) <- list(paste0("r.",r.bc(nrow(where))),
                            paste0("c.",r.bc(ncol(where))));
  } else {
    if (is.null(dimnames(where)[[1]])) {
      dimnames(where)[[1]] <- paste0("r.",r.bc(nrow(where)));
    }
  }
  ...
  if (nrow(where) < 1) {
    r.erreur(where,"'where' must have at least one valid row");
  }
  # sorting
  rrr <- r.text3ij2n(where[,1:2,drop=FALSE],text);
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.text3ij2n <- function(ij,text,flexible=TRUE)
...
# r.text3ij2n(c(1,3),aa);
# r.text3ij2n(c(2,1),aa);
# r.text3ij2n(rbind(c(1,3),c(2,1)),aa);
#ALIAS text-indexing
#REFERENCE
#SEE ALSO 
#CALLING
...
#--------------------------------------------
{
  ...
  if (is.matrix(ij)) {
    if (ncol(ij) != 2) {
      ...
      r.object9(ij,"numeric");
    }
  } else {
    r.object9(ij,"numeric",len=2);
    ij <- matrix(ij,ncol=2);
  }
  r.object9(text,"character");
  # degenerate case
  ...
  if (flexible) {
    ...
    r.object9(ij[,1],"numeric",con=c(1,length(text)));
    if ((any(ij[,2] < 1)) | (any(ij[,2]>nchar(text[ij[,1]])))) {
      r.erreur(cbind(ij,nchar(text[ij[,1]])),"Bad column numbering");
    }
  }
  # computing 
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.pause <- function(what="",mensaje=NULL,top=0,ans=NULL) 
#TITLE  pauses the program until an answer is given
#DESCRIPTION
# This function issues a r.pause with a message allowing 
...
#INPUTS
#[INPUTS]
#{what} << Short message commenting the r.pause>>
...
# by the user.
#EXAMPLE
# r.pause("Time for lunch!",top=NA);
# \dontrun{r.pause("Time for lunch?")}
# \dontrun{r.pause("Look at your results before the process continue",top=NULL)}
# \dontrun{r.pause("Can we continue?",top=c("n","N","no","NO"),ans=c("y","Y","yes","YES"))}
#REFERENCE
#SEE ALSO
#CALLING
#COMMENT
...
#--------------------------------------------
{
  ...
  if (!r.void9(mensaje)) {cat(">>> (",mensaje,")\n");}
  if (!r.void9(what))  {cat(">>> --------------> ",what,"\n");}
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.filter8text <- function(text,pattern,remove=TRUE,
...
# \samp{r.belong9} if it satisfies the conditions given by the other
# arguments, the character can be removed or selected.
#DETAILS
#KEYWORDS utilities
#INPUTS
...
#EXAMPLE
# r.filter8text(r.rbsa0$text2$v,"little",exact=FALSE,lower=TRUE,remove=FALSE)
# r.filter8text(r.rbsa0$text2$v,"On",exact=8,lower=TRUE,remove=FALSE)
# r.filter8text(r.rbsa0$text2$v,"On",        lower=TRUE,remove=FALSE)
# r.filter8text(r.rbsa0$text2$v,"On",exact=8,lower=TRUE,remove=TRUE)
#REFERENCE
#SEE ALSO r.belong9
#CALLING
#COMMENT
...
#--------------------------------------------
{
  ...
  # second checking
  if (monitor$chk$v) {
    # other checking are made within 'r.belong9'
  }
  # getting the conforming components
  coco <- vector("logical",length(text));
  for (ii in r.bf(text)) {
    coco[ii] <- r.belong9(pattern,text[ii],
...
nb <- max(0,round(nb));
res <- "";
for (jbd in r.bc(nb)) { res <- paste0(res,cha);}
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.form3justify <- function(chaine,
                          nbc=8,
                          format=3,
                          tronc=TRUE,
                          carac=" ",
...
#EXAMPLE
# r.form3justify("vers")
# r.form3justify("versification",5)
# r.form3justify(letters[1:5],format=2,carac="+");
#REFERENCE
#SEE ALSO r.text3preparation
#CALLING
#COMMENT
...
# checking
if (monitor$chk$v) {
    r.object9(chaine,c("character","numeric"),-1);
    r.object9(nbc,"integer",c(1,Inf));
}
# the null case
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.list4text <- function(text,content=c(1,length(text)),ccom="#",
                      lndel=c("{","}"),lvdel=c("<<",">>"))
...
# A named list.
#EXAMPLE
# r.list4text(c("{a}<<Il etait une fois>>",
#                "{b}<<   un petit et rouge chaperon>>{c}",
#                "<< pas bon>>",
#                "{d}    <<et le m'echant loup...>>"))
# r.list4text(c("{a}","<<","pour voir",">>"));
#REFERENCE
#SEE ALSO 
#CALLING
#COMMENT
...
#--------------------------------------------
{
  ...
  if (any(lndel=="")) {
    r.erreur(lndel,"bad labelling tags");
  }
  r.object9(lvdel,
             "character",len=2,
             mensaje="Bad 'lvdel' argument");
  if (any(lvdel=="")) {
    r.erreur(lndel,"bad valuing tags");
  }
  # extracting the list
  #
  # localizing the list delimiters
  oudon <- r.places4text6tags(text,tags=c(lndel[1],lndel[2],
                                        lvdel[1],lvdel[2]));
  ...
  for (ii in r.bc(nbi)) {
    ...
    if (length(v1) == 0) {
      ...
      if (length(w1)==0) { r.erreur(v1,"Internal Error 1!");}
      w1 <- oudon[w1[1],"co1"];
      ...
      if (length(v2) == 0) {
        ...
        if (length(w2)==0) { r.erreur(v2,"Internal Error 2!");}
        w2 <- oudon[w2[1],"co2"];
        ivalues[ii] <- list(r.text3stext(text,c(v1,w1,v2,w2)));
      }
    }
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.interv7belonging <- function(x,int,monitor=r.rbsa0$monitor$v)
...
#[INPUTS]
#{monitor} <<List providing the monitoring constants, see \code{r.rbsa0$monitor$v}
...
#EXAMPLE
# r.interv7belonging(1:5,1:2);
# r.interv7belonging(1:5,matrix(1:10,ncol=2));
#REFERENCE
#SEE ALSO
#CALLING
#COMMENT
...
#--------------------------------------------
{
    # checking
    if (monitor$chk$v) {
        r.object9(  x,"numeric",-1,mensaje="  'x' must be numeric");
        r.object9(int,"numeric",-1,mensaje="'int' must be numeric");
        if (!is.matrix(int)) {
            if (length(int) != 2) {
                r.erreur(int,"When 'int' is not a matrix, it must be a numeric(2)");
            }
            if (is.nan(diff(int))) {r.erreur(int,"This is not an accepted interval");}
            if (diff(int)<0) { r.erreur(int,"'int' does not define an interval!");}
        } else {
            if (ncol(int)!=2) {
                r.erreur(int,"When 'int' is a matrix, it must comprise 2 columnes");
            }
            ru <- int[,2] - int[,1];
            if (any(is.nan(ru))) { r.erreur(int,"Some rows are not accepted as intervals");}
            if (any((ru<0))) {
                r.erreur(int,"Not all rows of 'int' define an interval");
            }
        }
    }
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.text2vma <- function(cha,what=r.rbsa0$vma$v["v"],
                     xsep=r.rbsa0$sep1$v,nat="C",
                       monitor=r.rbsa0$monitor$v)
...
# \cr
# In fact \samp{r.rbsa0$vma$v} coding is used for the argument \samp{what}. 
...
#     \samp{r.rbsa0$vma$v["c"]}, \samp{r.rbsa0$vma$v["C"]},... but for the sake of the
...
#             is done with \samp{r.rbsa0$sep0$v}.
...
#####
## vectors
# r.text2vma(letters,"c");
# r.text2vma(letters,"C",xsep="e");
# r.text2vma(letters);
# r.text2vma(letters,"V");
# r.text2vma(letters,"u");
# r.text2vma(c(LETTERS,letters),r.rbsa0$vma$v["V"]);
# r.text2vma(c("A","a","B","b","C","c"),r.rbsa0$vma$v["U"]);
#####
## matrices
# r.text2vma(c(1:3,"//",4:6),r.rbsa0$vma$v["m"]);
# r.text2vma(c(1:3,"//",4:6),r.rbsa0$vma$v["M"]);
# r.text2vma(c(LETTERS[1:3],"//",1:3,"//",4:6),r.rbsa0$vma$v["n"]);
# r.text2vma(c(LETTERS[1:3],"//",1:3,"//",4:6),"N");
# r.text2vma(c("a",1:3,"//","b",4:6),"o");
# r.text2vma(c(c(LETTERS[1:3],"//","a",1:3,"//","b",4:6)),r.rbsa0$vma$v["p"]);
#####
## arrays
# r.text2vma(c(2:4,"//",1:24),"a");
# r.text2vma(c(2:4,"//","one","two","//",LETTERS[1:3],"//",
#          letters[1:4],"//",1:24),"A");
# r.text2vma(c(2:4,"//","one","two","//",LETTERS[1:3],"//",
...
{
# flexibility
if (r.void9(cha)) { cha <- character(0);}
# of use
ssep <- which(cha==xsep);
# checking
if (monitor$chk$v) {
    r.object9(cha,"character",-1,mensaje="cha must be a character");
    #
    r.object9(what,"character",1,
               mensaje="what (character(1)) must indicate the type of desired output");
    if (!(what %in% r.rbsa0$vma$v)) {
        r.erreur(what,"'what' not in the list of possible values...");
    }
    #
    r.object9(xsep,"character",1,mensaje="must indicate the character string of separation");
    #
    if (what %in% r.rbsa0$vma$v[c("V","U")]) {
        if ((length(cha) %% 2) != 0) {
            r.erreur(list(cha,what),"Here the length of 'cha' must be even");
        }
    }
    #
    if (what %in% r.rbsa0$vma$v[c("n","N","m","M","o","O")]) {
        if (length(ssep) >= 1) {
            nbc <- ssep[1] - 1;
            nbr <- (length(cha)+1) / (nbc+1);
            if ((nbr!=round(nbr)) | (length(cha) != (nbc*nbr+nbr-1))) {
                r.erreur(list(what,length(cha),nbc,nbr),"Dimensions not consistent");
            }
        }
    }
    #
    if (what %in% r.rbsa0$vma$v[c("p","P")]) {
        if (length(ssep) >= 1) {
            nbc <- ssep[1] + 1;
            X <- 2 + length(cha);
            if ((X %% nbc) != 0) {
                r.erreur(list(what,length(cha),nbc),"Dimensions not consistent");
            }
        }
    }
    #
    if (what %in% r.rbsa0$vma$v[c("a","A","b","B")]) { 
	# an array must be returned
        if (length(ssep)==0) {
            r.erreur(list(cha,xsep),"For arrays, dims must be provided");
        }
        didi <- cha[1:(ssep[1]-1)];
        didi <- as.numeric(didi);
        if (sum(is.na(didi))>0) {
            r.erreur(list(cha,xsep),"Dimensions are not numeric !");
        }
        if (any(didi<0)) {
            r.erreur(didi,"Negative dimensions were provided");
        }
        if (any(didi!=round(didi))) {
            r.erreur(didi,"Non integer dimensions were provided");
        }
        X <- length(didi) + 1 + prod(didi);
        if (what %in% r.rbsa0$vma$v[c("A","B")]) { X <- X + sum(didi) + length(didi); }
        if (what %in% r.rbsa0$vma$v[c("b","B")]) { X <- X + length(didi) + 1; }
	if (length(cha) != X) {
            r.erreur(list(cha,what,xsep),"Inconsistency for an array");
	}
    }
    #
    r.object9(nat,"character",1,mensaje="'nat' must be a character(1)");
}
#
res <- character(0);
#
if (what %in% r.rbsa0$vma$v["c"]) {
    # a single character must be returned
    # r.rbsa0$sep0$v is used as collapsor
    res <- paste(cha,collapse=r.rbsa0$sep0$v);
}
#
if (what %in% r.rbsa0$vma$v["C"]) {
    # a character must be returned
    # r.rbsa0$sep0$v is used as separator
    res <- paste(cha,collapse=r.rbsa0$sep0$v);
    res <- strsplit(res,xsep,fixed=TRUE)[[1]];
    if (length(res)>1) { for (ii in 2:length(res)) {
        res[ii  ] <- r.form3crop(res[ii  ],r.rbsa0$sep0$v,"",1);
        res[ii-1] <- r.form3crop(res[ii-1],"",r.rbsa0$sep0$v,1);
    }}
}
#
if (what %in% r.rbsa0$vma$v[c("v","V","u","U")]) {
    # a vector must be returned
    if (what %in% c(r.rbsa0$vma$v[c("v","u")])) {
        res <- cha;
    } else {
        nb <- floor(length(cha)/2);
        if (what == r.rbsa0$vma$v["V"]) { nam <- rep(c(TRUE,FALSE),each=nb);
...
#
if (what %in% r.rbsa0$vma$v[c("m","M","n","N","o","O","p","P")]) {
    # a matrix must be returned
    nbc <- ssep[1] - 1;
    cha <- c(cha,xsep);
    if (what %in% c(r.rbsa0$vma$v[c("p","P")])) {
        cha <- c(" ",cha);
    }
    if (what %in% r.rbsa0$vma$v[c("p","P")]) { add <- 1;
    } else { add <- 0; }
    cha <- matrix(cha,ncol=nbc+1+add,byrow=TRUE);
    cha <- cha[,-ncol(cha),drop=FALSE];
    if (what %in% r.rbsa0$vma$v[c("m","M")]) {
        res <- cha;
    }
    if (what %in% r.rbsa0$vma$v[c("n","N")]) {
        res <- cha[-1,,drop=FALSE];
        dimnames(res) <- list(NULL,cha[1,,drop=FALSE]);
    }
    if (what %in% r.rbsa0$vma$v[c("o","O")]) {
        res <- cha[,-1,drop=FALSE];
    ...
    if (what %in% r.rbsa0$vma$v[c("M","N","O","P")]) {
        res <- t(res);
    }
}
#
if (what %in% r.rbsa0$vma$v[c("a","A","b","B")]) {
    if (length(ssep) == 0) { r.erreur(cha,"For array, dimensions must be provided");}
    # an array must be returned
    didi <- cha[1:(ssep[1]-1)];
    didi <- as.numeric(didi);
    nbdi <- length(didi);
    #
    if (what == r.rbsa0$vma$v["a"]) { vvv <- cha[-(1:ssep[1])];}
    if (what == r.rbsa0$vma$v["A"]) { vvv <- cha[-(1:ssep[1+nbdi])];}
    if (what == r.rbsa0$vma$v["b"]) { vvv <- cha[-(1:ssep[2])];}
    if (what == r.rbsa0$vma$v["B"]) { vvv <- cha[-(1:ssep[2+nbdi])];}
    #
    res <- array(vvv,dim=didi);
    #
    if (what %in% r.rbsa0$vma$v[c("A","B")]) {
        ndi <- vector("list",0);
        for (jj in r.bf(didi)) {
            jjj <- jj + (what == r.rbsa0$vma$v["B"]);
    ...
    if (what %in% r.rbsa0$vma$v[c("b","B")]) { 
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.text3n2ij <- function(n,text,flexible=TRUE)
...
# In fact, the reverse of \samp{r.text3ij2n}.
#DETAILS
#KEYWORDS 
#INPUTS
...
#EXAMPLE
# aa <- c(paste(letters,collapse=""),paste(LETTERS,collapse=""),paste(0:9,collapse=""));
# r.text3n2ij(r.text3ij2n(c(1,3),aa),aa);
# r.text3n2ij(r.text3ij2n(c(2,1),aa),aa);
# r.text3n2ij(r.text3ij2n(rbind(c(1,3),c(2,1)),aa),aa);
#REFERENCE
#SEE ALSO 
#CALLING
#COMMENT
...
#--------------------------------------------
{
  ...
  if (flexible) {
    ...
    r.object9(n,"integer",con=c(1,sum(nchar(text))));
  }
  # initializing
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.text3stext <- function(text,sub=c(l1=1,p1=1,l2=Inf,p2=Inf))
...
# See the function \samp{r.text3interval} for details.>>
#VALUE
# The resulting sub-text
#EXAMPLE
# aa <- c(paste(letters,collapse=""),paste(LETTERS,collapse=""),paste(0:9,collapse=""));
# r.text3stext(aa);
# r.text3stext(aa,c(1,12,1,15));
# r.text3stext(aa,c(1,12,2,15));
# r.text3stext(aa,c(1,12,3,15));
# # In this last example, the last character of line 2 is kept (flexibility)!
# r.text3stext(aa,c(2,68,3,15));
#REFERENCE
#SEE ALSO
#CALLING
#COMMENT
...
#--------------------------------------------
{
  # checking finished by r.text3interval
  if (is.matrix(sub)) {
    if (nrow(sub)>1) {
      r.erreur(sub,"Only one interval is allowed");
    }
  }
  # getting the interval
  sub <- r.text3interval(text,sub);
  # extracting
  ii <- 1;
  if (is.na(sub[ii,1])) {
    res <- character(0);
  } else {
    res <- text[r.bd(sub[ii,1],sub[ii,3])];
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.text3acceptance <- function(line,posi,
                           b.accepted=NULL,a.accepted=NULL,
                           b.rejected=NULL,a.rejected=NULL,
                           b.line=TRUE,e.line=TRUE)
...
#EXAMPLE
# r.text3acceptance("Et pourquoi pas ?",c(1,3));
# r.text3acceptance("Et pourquoi pas ?",c(4,11),b.accepted=" ");
# r.text3acceptance("Et pourquoi pas ?",c(4,11),b.accepted=letters);
# r.text3acceptance("Et pourquoi pas ?",c(5,11),b.accepted=letters);
# \dontrun{r.text3acceptance("Et",1:2,b.accepted="letters",b.rejected=LETTERS)}
#REFERENCE
#SEE ALSO r.text3replace
#CALLING
#COMMENT This function was written to be used within \samp{r.text3replace}.
#FUTURE 
#AUTHOR J.-B. Denis
#CREATED 14_09_03
#REVISED 14_09_03
...
{
  # checking
  r.object9(line,"character");
  r.object9(posi,"integer",2);
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.text2file <- function(text,file,path="",
                      ap=FALSE,
                       monitor=r.rbsa0$monitor$v
                     )
#TITLE  writes a character onto a file
#DESCRIPTION
# The reverse operation of \samp{r.file2text}. This function is not
...
#EXAMPLE
# r.text2file(letters,"toto.txt");
# unlink("toto.txt");
# r.text2file(letters,character(0));
#REFERENCE
#SEE ALSO 
#CALLING
#COMMENT
...
if (monitor$chk$v) {
    r.object9(text,"character",-1,mensaje="'text' must be a character");
    #
    r.object9(file,"character",c(0,1),mensaje="file must indicate the name of one file");
    #
    if (!r.void9(path)) {
      path <- r.dipa(path);
      r.object9(path,"character",1,mensaje="path must indicate the name of one directory");
      #
      if (is.na(file.info(path)$size)) { r.erreur(path,"This directory seems not to exist.");}
    }
    #
    r.object9(ap,"logical",1,mensaje="'ap' must be a single logical");
}
#
# The returning case
if (r.void9(file)) { return(text);}
#
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.text3interval <- function(text,sub=c(l1=1,p1=1,l2=length(text),p2=Inf))
...
# aa <- c(paste(letters,collapse=""),paste(LETTERS,collapse=""),paste(0:9,collapse=""));
# r.text3interval(aa);
# r.text3interval(aa,c(1,1,12,15));
# r.text3interval(aa,c(1,2,12,15));
# r.text3interval(aa,c(10,2,1,15));
# r.text3interval(aa,c(1,3,12,15));
# r.text3interval(aa,c(2,3,68,15));
# r.text3interval(aa,matrix(c(1,1,12,15,
#                           1,2,12,15,
#                           10,2,1,15,
#                           1,3,12,15,
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.form3parag <- function(texte,titre=-1,
...
#{monitor} << List of monitoring constants, see \samp{r.rbsa0$monitor$v} to
#             know its structure.>>
#{imp} << Printing is performed and nothing is returned.
#                If FALSE, the character string is returned 
#                (including possible new lines)>>
...
#EXAMPLE
# r.form3parag(c("My Title","My important first sentence.","Last one!"));
# r.form3parag(r.rbsa0$text2$v,wid=20);
#REFERENCE
#SEE ALSO
#CALLING {r.form3repeat}
#COMMENT
...
# checking
if (monitor$chk$v) {
    r.object9(texte,"character");
    r.object9(wid,"numeric",1,c(1,Inf));
    if (!is.null(fli)) {
      r.object9(fli,"numeric",3,c(0,Inf));
    }
    r.object9(sep,"integer",1,c(1,Inf));
    r.object9(jus,"integer",1,c(1,3));
    r.object9(trunc,"logical",1);
    r.object9(ed,"character",1);
    r.object9(ef,"character",1);
    r.object9(imp,"logical",1);
}
sep <- min(max(0,sep),5);
if (titre > -2) {
    # the possible title
    res <- r.form3title(texte[1],empha=titre,indent=nchar(ed),imp=imp);
    if (imp) { cat(r.form3repeat("\n",sep+1));
    } else { res <- c(res,r.form3repeat("\n",sep));}
    texte <- texte[-1];
...
for (i in r.bf(texte)) {
    ...
    while (length(mots) > 0) {
        ...
        spfl <- ((nlig==1)&(!r.void9(fli)));
        if (spfl) {
            if (monitor$chk$v) {
                r.object9(fli,"numeric",c(3,Inf));
            }
            wiid <- fli[2];
            lili <- r.form3repeat(" ",fli[1]);
            trop <- fli[3];
        } else {
            wiid <- wid;
            #lili <- character(0);
	...
	#cat("<<",length(mots),">>\n");
        ...
        #cat("{{",nchar(lili),lili,"}}\n");
        lili <- r.form3justify(lili,wid-trop,jus,trunc);
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.display8k <- function(constants,what="exhibit")
...
#EXAMPLE
# r.display8k(r.rbsa0)
# r.display8k(r.rbsa0$monitor$v,"v")
#REFERENCE
#SEE ALSO
#CALLING
#COMMENT
...
{
  # checking
  r.object9(what,"character",1,mensaje="Awaiting for a character(1)");
  what <- tolower(what);
  what <- substr(what,1,1);
  r.object9(what,"character",1,c("d","v","n","e"),
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
r.rbsa7list9 <- function(lili,monitor=r.rbsa0$monitor$v)
...
# To be handled by \samp{list2file} or \samp{r.file2list} functions, a list must
# rbsa-compatible.
#DETAILS
#KEYWORDS IO
#INPUTS
...
#EXAMPLE
# r.rbsa7list9(r.rbsa0$lis1$v);
# r.rbsa7list9(list(r.rbsa0$lis1$v));
# r.rbsa7list9(list(r.rbsa0$lis1$v,nu=NULL));
#REFERENCE
#SEE ALSO 
#CALLING
#COMMENT
...
for (ii in r.bc(nrow(eee))) {
    ...
    coco <- r.get8comp7list(lili,eee[ii,,drop=FALSE],monitor=monitor);
