...
#{griddim}<<A vector of two integers, indicating the size of the grid of the graph. If \code{NULL}, the grid is calculated to produce a "nice" graph.>>
#{log}<<If \code{TRUE}, the data will be log transformed.>>
#NOTE
#This function may be used on a \code{mccut} object only if a \code{summary.mc} function was used in the third block of the \code{\link{evalmccut}} call.
#The values used as \code{probs} arguments in \code{converg} should have been used in the \code{summary.mc} function of this third block.
#DETAILS
#If the node is of type \code{"V"}, the running mean, median and \code{probs}
#quantiles according to the variability dimension will be provided. If the node is of type \code{"VU"} and \code{margin="var"},
#this graph will be provided on one simulation in the uncertainty dimension (chosen by \code{iter}).</>
#If the node is of type \code{"U"} the running mean, median and \code{lim}
