...
#NAME mcnode
#DESCRIPTION
# Creates a \code{mcnode} object from a vector, an array or a \code{mcnode}.
#KEYWORDS methods
#INPUTS
#{data}<<The numeric/logical vector/matrix/array of data or the \code{mcnode} object.>>
#[INPUTS]
#{type}<<The type of node to be built. By default, a \code{"V"} node.>>
#{nsv}<<The variability dimension (\code{type="V"} or \code{type="VU"}) of the node.
#By default: the current value in \code{\link{mc.control}}>>
#{nsu}<<The uncertainty dimension (\code{type="U"} or \code{type="VU"}) of the node.
#By default: the current value in \code{\link{mc.control}}>>
...
#DETAILS
#A \code{mcnode} object is the basic element of a \code{\link{mc}} object.
#It is an array of dimension \code{(nsv x nsu x nvariates)}. Four types of \code{mcnode} exists:</>
#{*}<<\code{"V" mcnode}, for "Variability", are arrays of dimension \code{(nsv x 1 x nvariates)}. The alea in the data should
#reflect variability of the parameter.>>
#{*}<<\code{"U" mcnode}, for "Uncertainty", are arrays of dimension \code{c(1 x nsu x nvariates)}. The alea in the data should
...
#If the final dimension of the node is \code{(nsv x nsu x nvariates)} (with \code{nsv = 1} and \code{nsu = 1} for \code{"0"} nodes,
#\code{nsu = 1} for \code{"V"} nodes
#and \code{nsv = 1} for \code{"U"} nodes), \code{mcdata} accepts :
#{*}<<Vectors of length \code{1} (recycled on all dimensions), vectors of length \code{(nsv * nsu)} (filling first the dimension of variability, then
#the dimension of uncertainty then recycling on nvariates),
#or vectors of length \code{(nsv * nsu * nvariates)} (filling first the dimension of variability, then the uncertainty,
#then the variates).>>
#{*}<<Matrixes of dimensions \code{(nsv x nsu)}, recycling on variates.>>
#{*}<<Arrays of dimensions \code{(nsv x nsu x nvariates)} or \code{(nsv x nsu x 1)}, recycling on variates.>>
#{*}<<For \code{data} as \code{mcnode}, recycling is dealt to proper fill the array:>>
#{#}<<a \code{"V"} node accepts a \code{"0"} node of dimension \code{(1 x 1 x nvariates)} (recycling on variability)
#or of dimension \code{(1 x 1 x 1)} (recycling on variability and variates),
#or a \code{"V"} node of dimension \code{(nsv x 1 x nvariates)}
#or \code{(nsv x 1 x 1)} (recycling on variates),>>
#{#}<<a \code{"U"} node accepts a \code{"0"} node of dimension \code{(1 x 1 x nvariates)} (recycling on uncertainty)
#or of dimension \code{(1 x 1 x 1)} (recycling on uncertainty and variates),
#or a \code{"U"} node of dimension \code{(1 x nsu x nvariates)},
#or \code{(1 x nsu x 1)} (recycling on variates),>>
#{#}<<a \code{"VU"} node accepts a \code{"0"} node of dimension \code{(1 x 1 x nvariates)} (recycling on varaiability and uncertainty)
#or of dimension \code{(1 x 1 x 1)} (recycling on variability, uncertainty and variates),
#a \code{"U"} node of dimension \code{(1 x nsu x nvariates)}(recycling "by row" on the variability dimension),
#or of dimension \code{(1 x nsu x 1)}(recycled "by row" on the variability dimension then on variates),
#a \code{"V"} node of dimension \code{(nsv x 1 x nvariates)}(recycling on the uncertainty dimension)
#or of dimension \code{(nsv x 1 x 1)}(recycled on the uncertainty dimension then on variates),
#and a \code{"VU"} node of dimension \code{(nsv x nsu x nvariates)} or of dimension \code{(nsv x nsu x 1)} (recycling on variates).>>
#{#}<<a \code{"0"} node accepts a \code{"0"} node of dimension \code{(1 x 1 x nvariates)} or
#\code{(1 x 1 x 1)} (recycling on variates).>>
#
#
#SEE ALSO
#\code{\link{mcstoc}} to build a stochastic \code{mcnode} object, \code{\link{mcprobtree}} to build a
...
#To build a correlation structure between \code{mcnode}: \code{\link{cornode}}.</>
##To apply a function on a \code{mcnode} object: \code{\link{mcapply}}</>
#To study \code{mcnode} objects: \code{\link{print.mcnode}}, \code{\link{summary.mcnode}}, \code{\link{plot.mcnode}},
#\code{\link{converg}}, \code{\link{hist.mcnode}}</>
#To modify \code{mcnode} objects:
##\code{\link{subset.mc}}
#\code{\link{NA.mcnode}}</>
##To transform \code{mc} objects in vector or matrix: \code{\link{unmcnode}}.
#EXAMPLE
#oldvar <- ndvar()
#oldunc <- ndunc()
#ndvar(3)
