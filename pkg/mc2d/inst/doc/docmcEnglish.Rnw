...
\begin{itemize}
\item A medium level of experience in R. Please refer to the Manual {}``An
...
\begin{itemize}
\item provides additional probability distributions;
\item provides tools to construct One-Dimensional and Two-Dimensional Monte-Carlo
Simulations;
\item provides tools to analyse One-Dimensional and Two-Dimensional Monte-Carlo
Simulations.
\end{itemize}
In a previous version, some tools to fit parametric distributions
to data were included. Because these functions are useful for other
purposes, they have been moved to a separate package called \texttt{fitdistrplus}.
Both the \texttt{mc2d} and the \texttt{fitdistrplus} packages are
available at the URL \url{https://r-forge.r-project.org/projects/riskassessment/}. 

\texttt{mc2d} was built for QRA in the Food Safety domain but it can
be used in other QRA domains. 
...
temporal, geographical and/or individual heterogeneity of the risk
...
reflecting \textquotedbl{}variability\textquotedbl{} and the distributions
representing \textquotedbl{}uncertainty\textquotedbl{} are sampled
separately in the simulation, so that \textquotedbl{}variability\textquotedbl{}
and \textquotedbl{}uncertainty\textquotedbl{} in the output may be
estinated separately. It may be described as following (see Figure
\ref{fig:Mc2d}): 
\begin{enumerate}
\item The parameters of the model should be divided into three categories:
the parameters whose distributions reflect {}``variability only'',
hereinafter denoted as {}``variable parameters'', the parameters
whose distributions reflect {}``uncertainty only'', denoted as {}``uncertain
parameters'' and the parameters whose distributions reflect both
uncertainty and variability. For this latter category, a hierarchical
structure, using {}``hyper-parameters'', should be specified: if
...
is some associated uncertainty. As an example, one should be able
to specify a relationship such as $X\left|a,b\right.\sim N\left(a,b\right)$,
where the specified normal distribution represents variability in
$x$ conditional upon parameters $a$ and $b$. Hyperdistributions,
such as $a\sim Unif\left(l_{a},u_{a}\right)$and $b\sim Unif\left(l_{b},u_{b}\right)$,
represent the uncertainty in the parameters $a$ and $b$;
...
leading to $N_{u}$ sets of statistics; 
\item As output, the $50^{\text{th}}$ percentile (median) of each statistic
is used as a point estimate of this statistic; the $2.5^{\text{th}}$
...
\includegraphics[scale=0.5,bb = 0 0 200 100, draft, type=eps]{mc2dsaumon}
\par\end{centering}
...
\textquotedbl{}\texttt{mc2d}\textquotedbl{} is a set of R functions
that will help to develop such two-dimensional Monte-Carlo simulations.
The main difference from the procedure described above is that \texttt{mc2d}
...
\item We assume that no bacterial growth occurs in storage, since the product
...
\end{itemize}
\item The variability in steak serving sizes $s$ for <3 year children was
estimated in a consumption survey. The {}``best fit'' to the data
was a gamma distribution with parameters: $shape=3.93,\: rate=0.0806$.
...
where $emp\left(\boldsymbol{X},\boldsymbol{P}\right)$ is an empirical
distribution wherein each value $X_{i}$ is associated with a probability
$P_{i}$.We will use a \textquotedbl{}classical\textquotedbl{} one
...
which is a function of $c$, $i$ and $s,$ will be both variable
\emph{and} uncertain.
...
parameter distribution represents uncertainty (\texttt{type}=\textquotedbl{}\texttt{U}\textquotedbl{}),
variability (\texttt{type}=\textquotedbl{}V\textquotedbl{}), or both
(\texttt{type}=\textquotedbl{}\texttt{VU}\textquotedbl{}).
...
is now uncertain. The median of the 100 simulations leads to a \textquotedbl{}best
estimate\textquotedbl{}of 0.0445, with a 95\% \textquotedbl{}credible
interval\textquotedbl{} of {[}0.191, 0.0730{]}.
...
an empirical distribution from data , the {}``\texttt{fitdistrplus}''
...
of your R session (\textquotedbl{}\texttt{library(mc2d)}\textquotedbl{}).
...
\footnote{In this section, we will only consider \texttt{mcnode}s with $nvariates=1$.%
}. Four types of \texttt{mcnode} exist: 
\begin{itemize}
\item \textquotedbl{}\texttt{V}\textquotedbl{} \texttt{mcnode}, for \textquotedbl{}\emph{Variability}\textquotedbl{},
is an array of dimension $\left(nsv\times1\times nvariates\right)$.
The distribution represents variability in the parameter; 
\item \textquotedbl{}\texttt{U}\textquotedbl{} \texttt{mcnode}, for \textquotedbl{}\emph{Uncertainty}\textquotedbl{},
is an array of dimension $\left(1\times nsu\times nvariates\right)$.
The distribution represents uncertainty in the parameter. 
\item \textquotedbl{}\texttt{VU}\textquotedbl{} \texttt{mcnode}, for \textquotedbl{}\emph{Variability
and Uncertainty}\textquotedbl{}, is an array of dimension $\left(nsv\times nsu\times nvariates\right)$.
...
\item Additionally, a \textquotedbl{}\texttt{0}\textquotedbl{} \texttt{mcnode}
is also defined . \textquotedbl{}\texttt{0}\textquotedbl{} stands
for \textquotedbl{}Neither Variability or Uncertainty\textquotedbl{}.
Such nodes are arrays of dimension $\left(1\times1\times nvariates\right)$.
No uncertainty or variability is considered for these nodes. A \textquotedbl{}\texttt{0}\textquotedbl{}
...
\begin{figure}
\noindent \begin{centering}
\includegraphics[scale=0.4,bb = 0 0 200 100, draft, type=eps]{Illustration}
\par\end{centering}
...
There are 5 ways to construct an \texttt{mcnode} object:
\begin{enumerate}
...
from other \texttt{mcnodes} using a probability tree;
...
\footnote{as is standard in R, most arguments have logical default values and
will be infrequently modified.%
}:

\begin{singlespace}
\noindent \begin{flushleft}
\texttt{mcstoc(func=runif, type=c(\textquotedbl{}V\textquotedbl{},
\textquotedbl{}U\textquotedbl{}, \textquotedbl{}VU\textquotedbl{},
\textquotedbl{}0\textquotedbl{}), ..., nsv=ndvar(), nsu=ndunc(), nvariates=1,
outm=\textquotedbl{}each\textquotedbl{}, nsample=\textquotedbl{}n\textquotedbl{},
...
of the following arguments should be named};
\item \texttt{nsv} and \texttt{nsu} are the number of samples needed in
the variability and uncertainty dimension, respectively. By default,
...
and \texttt{lsup}. This function is not valid for every distribution
...
\footnote{Note that the mean and the standard deviation of the untruncatednormal
...
The multinomial distribution has been adapted (vectorized): \texttt{rmultinomial}
...
Another way to construct an \texttt{mcnode} object is \emph{via} the
\texttt{mcdata} function, when data are available.

\begin{singlespace}
\noindent \begin{flushleft}
\texttt{mcdata(data, type=c(\textquotedbl{}V\textquotedbl{}, \textquotedbl{}U\textquotedbl{},
\textquotedbl{}VU\textquotedbl{}, \textquotedbl{}0\textquotedbl{}),
nsv=ndvar(), nsu=ndunc(), nvariates=1, outm=\textquotedbl{}each\textquotedbl{})}
\par\end{flushleft}
\end{singlespace}
...
example places a \texttt{TRUE} value in a \textquotedbl{}\texttt{U}\textquotedbl{}
...
are as follows (illustrated here with a {}``+'')%
\footnote{These rules are not the standard R rules for recycling.%
}:
\begin{itemize}
\item \textquotedbl{}\texttt{0}\textquotedbl{} + \textquotedbl{}\texttt{0}\textquotedbl{}
= \textquotedbl{}\texttt{0}\textquotedbl{};
\item \textquotedbl{}\texttt{0}\textquotedbl{} + \textquotedbl{}\texttt{V}\textquotedbl{}
= \textquotedbl{}\texttt{V}\textquotedbl{}
\item \textquotedbl{}\texttt{0}\textquotedbl{} + \textquotedbl{}\texttt{U}\textquotedbl{}
= \textquotedbl{}\texttt{U}\textquotedbl{};
\item \textquotedbl{}\texttt{0}\textquotedbl{} + \textquotedbl{}\texttt{VU}\textquotedbl{}
= \textquotedbl{}\texttt{VU}\textquotedbl{};
\item \textquotedbl{}\texttt{V}\textquotedbl{} + \textquotedbl{}\texttt{V}\textquotedbl{}
= \textquotedbl{}\texttt{V}\textquotedbl{};
\item \textquotedbl{}\texttt{V}\textquotedbl{} + \textquotedbl{}\texttt{U}\textquotedbl{}
= \textquotedbl{}\texttt{VU}\textquotedbl{}: the {}``\texttt{U}''
mcnode is recycled by row, the \textquotedbl{}\texttt{V}\textquotedbl{}
mcnode is recycled in the standard manner by column;
\item \textquotedbl{}\texttt{V}\textquotedbl{} + \textquotedbl{}\texttt{VU}\textquotedbl{}
= \textquotedbl{}\texttt{VU}\textquotedbl{}: the {}``\texttt{V}''
mcnode is recycled in the standard manner by column;
\item \textquotedbl{}\texttt{U}\textquotedbl{} + \textquotedbl{}\texttt{U}\textquotedbl{}
= \textquotedbl{}\texttt{U}\textquotedbl{};
\item \textquotedbl{}\texttt{U}\textquotedbl{} + \textquotedbl{}\texttt{VU}\textquotedbl{}
= \textquotedbl{}\texttt{VU}\textquotedbl{}: the {}``\texttt{U}''
mcnode is recycled by row;
\item \textquotedbl{}\texttt{VU}\textquotedbl{} + \textquotedbl{}\texttt{VU}\textquotedbl{}
= \textquotedbl{}\texttt{VU}\textquotedbl{}
\end{itemize}
Thus, in our example:
...
The \texttt{mcprobtree} function can be used if a {}``probability
tree'' is needed to construct an \texttt{mcnode}. Assume that the
...
type=\textquotedbl{}U\textquotedbl{}, values=c(0,1), prob=c(75,25))}
or \texttt{whichdist <- mcstoc(rbern,type=\textquotedbl{}U\textquotedbl{},prob=0.25)}%
}:
...
people who consume rare ground beef alsoconsume larger serving sizes.
...
One can re-run the model with various dimensions or random seeds in
one line:
...
\texttt{evalmccut} evaluates a 2-dimensional Monte-Carlo model (written
...
\noindent \begin{flushleft}
\texttt{hist (x, griddim = NULL, xlab = names(x), ylab = \textquotedbl{}Frequency\textquotedbl{},
main = \textquotedbl{}\textquotedbl{}, ...)}
\par\end{flushleft}
...
\noindent \begin{flushleft}
\texttt{plot (x, prec = 0.01, stat = c(\textquotedbl{}median\textquotedbl{},
\textquotedbl{}mean\textquotedbl{}), lim = c(0.025,0.975), na.rm =
TRUE, griddim = NULL, xlab = NULL, ylab = \textquotedbl{}Fn(x)\textquotedbl{},
main = \textquotedbl{}\textquotedbl{}, draw = TRUE, ...)}
\par\end{flushleft}
...
\noindent \begin{flushleft}
\texttt{tornado(x, output=length(x), use=\textquotedbl{}all.obs\textquotedbl{},
method=c(\textquotedbl{}spearman\textquotedbl{}, \textquotedbl{}kendall\textquotedbl{},\textquotedbl{}pearson\textquotedbl{}),
lim=c(0.025, 0.975))}
\par\end{flushleft}
...
use = \textquotedbl{}all.obs\textquotedbl{}, method=c(\textquotedbl{}spearman\textquotedbl{},\textquotedbl{}kendall\textquotedbl{},\textquotedbl{}pearson\textquotedbl{}),
...)}
\par\end{flushleft}
...
The output shows the impact of the uncertain nodes (type \textquotedbl{}U\textquotedbl{}
nodes) and some statistics (mean, median and, here, the $99^{\text{th}}$percentile)
calculated on the variability dimension (type \textquotedbl{}V\textquotedbl{}
and type \textquotedbl{}VU\textquotedbl{} nodes) of some output statistics
.
...
to these objects. The \textquotedbl{}\texttt{\$}\textquotedbl{} function
...
of steak eaten by 500 babies sampled from these 500 families.
...
Assume that each member of these families eats a {}``normal'' distribution
...
well as \texttt{sigma}%
\footnote{Caution: the use of a varying \texttt{sigma} can make the analysis
very slow.%
...
Finally, multivariate nodes may be useful to derive a nonparametric
...
for each variate, or, as in the example, a vector of functions that
...
dimension are as follows: the recycling will be done from \texttt{nvariates=1}
...
options; ii) transfer these options into the final risk estimate.
...
As a conclusion, this \textquotedbl{}third\textquotedbl{} dimension
is highly flexible...
...
but we will use the \textquotedbl{}\texttt{fitdistrplus}\textquotedbl{}
...
with \textquotedbl{}\texttt{0}\textquotedbl{}and \textquotedbl{}non-\texttt{0}\textquotedbl{}
data. 
...
of \textquotedbl{}0\textquotedbl{}and \textquotedbl{}non-0\textquotedbl{}
distributions:
...
We first define a function \textquotedbl{}\texttt{DR}\textquotedbl{}
