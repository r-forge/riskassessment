...
\[ X\left|a,b\right.\sim N\left(a,b\right) \]
...
parameter distribution represents uncertainty (\texttt{type}=\texttt{U}),
variability (\texttt{type}=V), or both
(\texttt{type}=\texttt{VU}).
...
It is an array of dimension \texttt{nsv} * \texttt{nsu} * \texttt{nvariates}
...
\begin{figure}
\noindent \begin{centering}
\includegraphics[angle=270, width=.5\textwidth]{Illustration}
\par\end{centering}
...
= \texttt{0};
\item \texttt{0} + \texttt{V}
= \texttt{V}
\item \texttt{0} + \texttt{U}
= \texttt{U};
\item \texttt{0} + \texttt{VU}
= \texttt{VU};
\item \texttt{V} + \texttt{V}
= \texttt{V};
\item \texttt{V} + \texttt{U}
= \texttt{VU}: the \texttt{U}
mcnode is recycled by row, the \texttt{V}
mcnode is recycled in the standard manner by column;
\item \texttt{V} + \texttt{VU}
= \texttt{VU}: the \texttt{V}
mcnode is recycled in the standard manner by column;
\item \texttt{U} + \texttt{U}
= \texttt{U};
\item \texttt{U} + \texttt{VU}
= \texttt{VU}: the \texttt{U}
mcnode is recycled by row;
\item \texttt{VU} + \texttt{VU}
= \texttt{VU}
\end{itemize}
Thus, in our example:
...
The output shows the impact of the uncertain nodes (type U
nodes) and some statistics (mean, median and, here, the $99^{th}$percentile)
calculated on the variability dimension (type V
and type VU nodes) of some output statistics
.
...
two-dimensional simulations. Nevertheless,
\texttt{mc2d} is currently under development:
...
Please refer any comments or bugs to rpouillot@yahoo.fr.
