...
rs003k("reset"); rsba3k("reset"); rsgn3k("reset");

#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
rsbn3k <- function(whi)
#TITLE assigns the constants for the rsbn layer
#DESCRIPTION (bn)
...
#DETAILS
# The constant rbsb.pgr0 is used to build the graph
#PKEYWORDS bn
#KEYWORDS
#INPUTS
...
if (rbsb.mck) {
    ...
    check4tyle(nv,"character",1,"'nv' must be 'character'");
    if (!(nv %in% c("n","v"))) {
...
{
# checking
if (rbsb.mck) {check4valid(valid8gn(gn));}
if (cycle8bgn(gn)) {
...
#EXAMPLE
# rsbn3k("RESET"); # needed when R is checking the examples
# bn2pam(rbsb.bn3,"v");
# bn2pam(rbsb.bn3,"n");
#REFERENCE
#FUTURE 
...
if (rbsb.mck) {
    check4valid(valid8bn(bn));
    check4tyle(nv,"character",1,"'nv' must be 'character(1)'");
    if (!(nv %in% c("n","v"))) {
...
lcomp <- FALSE;
lparent <- rbsb.cha0;
if (tolower(rbsb.l_a[ltype,"lparent"]) == "yes") {
    for (ipp in vk) {
...
if (rbsb.mck) {
    check4valid(valid8bn(bn));
    check4tyle(of,"character",1,"'of' must be 'character(1)'");
    if (!(of %in% c("n","v"))) {
...
if (of=="v") {
    ...
    # the node case
    res <- lapply(sj(length(bn@nom)),function(x){character(0);});
...
#EXAMPLE
# rsbn3k("RESET"); # (only for R checking)
# children8bn(rbsb.bn3,"n");
# children8bn(rbsb.bn3,"v");
#REFERENCE
#SEE ALSO
#CALLING
#COMMENT
...
if (rbsb.mck) {
    check4valid(valid8bn(bn));
    check4tyle(of,"character",1,"'of' must be 'character'");
    if (!(of %in% c("n","v"))) {
        erreur(of,"'of' must be 'n' or 'v'");
    }
}
...
papa <- bn2pam(bn,of);
# preparing the list
res <- lapply(sj(nrow(papa@rlt)),function(x){character(0);});
names(res) <- dimnames(papa@rlt)[[1]];
for (ii in sj(nrow(papa@rlt))) {
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
modify4bn <- function(bn,pg=rbsb.pgr0)
...
#EXAMPLE
# rsbn3k("RESET"); # (only for R checking)
# \dontrun{plot8gene(rbsb.gn7,nbrow=4,nbcol=3);}
#REFERENCE
#SEE ALSO
...
#EXAMPLE
# rsbn3k("RESET"); # (only for R checking)
# \dontrun{plot8path(rbsb.gn7,"A","F",nbrow=1,nbcol=1);}
# \dontrun{plot8path(rbsb.gn7,"A","B",nbrow=1,nbcol=1);}
#REFERENCE
#SEE ALSO
#CALLING 
#COMMENT
...
#EXAMPLE
# rsbn3k("RESET"); # (only for R checking)
# print(rcatbn(rbsb.gn7),quoi="l",qui=9,proba=TRUE);
#REFERENCE
#SEE ALSO rnorbn
#CALLING
#COMMENT
...
# print(anode4bn(rbsb.bn2,"sup",c("A","B[b]")));
#REFERENCE
#SEE ALSO and4bn
#CALLING {and4bn}
#COMMENT
...
#EXAMPLE
# rsbn3k("RESET"); # (only for R checking)
# connect8bgn(rbsb.bn0);
# connect8bgn(rbsb.bn3);
# connect8bgn(rbsb.gn1);
# connect8bgn(rbsb.gn6);
#REFERENCE
#SEE ALSO
#CALLING
#COMMENT
...
if (nba==0) {
    # no arcs
    return(list(nodes=structure(sj(nbn),.Names=names(bgn@nom@x)),
...
#EXAMPLE
# rsbn3k("RESET"); # For R checking compliance
# write8bn(rbsb.bn3);
# readLines(rbsb.fou);
# unlink(rbsb.fou);
#REFERENCE
