...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
rsgn3k <- function(whi)
#TITLE assigns the constants for the rsgn layer
#DESCRIPTION (gn)
...
#
    ...
    assign("rbsb.gn5",new("gn",
                 ...
                 arc=new("arc",nbn=2,fle=matrix(c(1,2,
                                                  rep(211,1)),1)),
                 pgr=rbsb.pgr0),pos=.GlobalEnv);
    assign("rbsb.gn6",new("gn",
                 description=char2des("Example 6 of /gn/"),
                 nom=new("nom",x=char2list(letters[1:6],nam=LETTERS[1:6])), 
                 item="n",
...
{
# checking
if (rbsb.mck) {check4valid(valid8gn(x));}
...
{
# checking
if (check) { if (rbsb.mck) {check4valid(valid8gn(gn));}}
# creating
res <- nanv(gn,gn@item);
# returning
res;
...
#EXAMPLE
# rsgn3k("RESET"); # needed only for R checking, to be forgotten
# cycles4gn(rbsb.gn7); # returns the null /gn/
#REFERENCE
#SEE ALSO
#CALLING
#COMMENT
...
#EXAMPLE
# rsgn3k("RESET"); # needed only for R checking, to be forgotten
# path4gn(rbsb.gn7,"E","J",3,FALSE);
# plot(path4gn(rbsb.gn7,"A",LETTERS[1:10],3,FALSE));
#REFERENCE
#SEE ALSO
#CALLING
#COMMENT
...
#EXAMPLE
# rsgn3k("RESET"); # for R checking need
...
for (ll in sjl(li$arc)) {
    ...
    if (length(lulu) > 1) {
        ...
        nunu <- as.vector(outer(lulu,itou,"==") %*% sjl(itou));
...
# The generated list
#EXAMPLE
# rsgn3k("RESET"); # For R checking compliance
# gn2list(rbsb.gn3);
#REFERENCE
#SEE ALSO
#CALLING
...
#EXAMPLE
# rsgn3k("RESET"); # needed only for R checking, to be forgotten
# neighbours8gn(rbsb.gn7,"parents");
# neighbours8gn(rbsb.gn7,"children");
#REFERENCE
#SEE ALSO
#CALLING
#COMMENT
