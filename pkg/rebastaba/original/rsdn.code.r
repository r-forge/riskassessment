...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
rsdn3k <- function(whi)
#TITLE assigns the constants for the rsdn layer
#DESCRIPTION (dn)
...
#
    ...
    assign("rbsb.mnd",10,pos=.GlobalEnv);
...
# of the candidate subset is appended to the file of name 'rbsb.efi'.\cr
...
# draw8empidata(X,E,win,nat=structure("conti",.Names="A"),bnid="rbsb.bn2");
#REFERENCE
#SEE ALSO
#CALLING
#COMMENT
...
if (rbsb.mck) {
    ava <-  dimnames(E)[[2]];
    if (!all(vari %in% c(ava,names(rbsb.ena)))) {
...
#
uvari <- setdiff(vari,names(rbsb.ena));
E <- E[,unique(c(vparents,uvari)),drop=FALSE];
#
# Giving the general trait of the drawing in rbsb.efi file
#
sink(rbsb.efi,TRUE);
dvar <- form3liste(vari,OPA="[ ",CPA=" ]",opa="",cpa="",sep=" / ")
form3titre(paste("/bn/ =",bnid," |  variable(s) =",dvar),10);
form3titre(paste("At time",now()),0);
form3titre(paste("empidata of size",nbs),0);
form3titre(paste("The draw is made from a population of size",nbe),0);
form3titre("The draw is monitored from the following /win/:",1);
print8win(win);
if (rbsb.esu) {
    form3titre("As rbsb.esu is TRUE selected subsets will be introduced for each draw\n",0);
} else {
    form3titre("As rbsb.esu is FALSE selected subsets will not be introduced for each draw\n",0);
}
sink();
#
# adding the computed variable to the base if necessary
#
if (names(rbsb.ena)[1] %in% vari) {
    # subset sizes are required
    xxx <- rep(nrow(E),nrow(E));
    E <- cbind(E,xxx);
    dimnames(E)[[2]][ncol(E)] <- names(rbsb.ena)[1];
    nat <- c(nat,"integ");
    names(nat)[length(nat)] <- names(rbsb.ena)[1];
}
if (names(rbsb.ena)[2] %in% vari) {
    # maximum distances are required
    xxx <- rep(NA,nrow(E));
    E <- cbind(E,xxx);
    dimnames(E)[[2]][ncol(E)] <- names(rbsb.ena)[2];
    nat <- c(nat,"conti");
    names(nat)[length(nat)] <- names(rbsb.ena)[2];
}
#
...
#EXAMPLE
# rsdn3k("RESET"); # for the sake of R checking
...
# checking that rbsb.ena variables are not introduced
if (length(intersect(names(rbsb.ena),names(EE)))>0) {
    erreur(names(rbsb.ena),
...
if (win@rty[2] %in% c("systematic")) {
    ...
    if (length(win@rmo)==0) { moni <- sj(nrow(EE));
    ...
    if (win@rty[1] == "1") {
        ...
        for (ii in sj(ncol(EE))) {
	    uu <- systematic(sj(nrow(EE)),nbs,freq);
	    res[,ii] <- EE[uu,ii];
        }
    }
    #
...
if (!fait) {
	...
	if (length(win@rwg) > 0) {
        ...
        for (ii in sjl(wiwi@swg)) { if (wiwi@swg[ii]<0) {
...
#        \code{rbsb.ena[1]} computed if asked).\cr
#EXAMPLE
# rsdn3k("RESET"); # to comply R checking
# EE <- data.frame(A=1:100,c=100:1); # the empirical distribution
# win <- new("win",nat=structure("conti",.Names="c"),swg=structure(1,.Names="A"),skk=0,sdi=c(0,1),snb=c(1,20));
...
if (win@rty[2]=="random") {
    ...
    if (win@rty[1] == "1") {
        ...
        for (ii in sj(ncol(EE))) { res[1,ii] <- EE[uu[ii],ii];}
        if (win@rty[1]=="0") {
...
if (win@rty[2] %in% c("median","mean","quantile","stdev")) {
    ...
    if (win@rty[1] == "1") {
        ...
        for (ii in sj(ncol(EE))) { res[1,ii] <- sta(EE[,ii]);}
        if (win@rty[1]=="0") {
            res <- draw8sele(res,EE,win@rwg,win@rkk,c(0,Inf),c(1,1),nat);
        }
    }
...
if (names(rbsb.ena)[1] %in% names(EE)) {
    res[1,names(rbsb.ena)[1]] <- nrow(EE);
}
# returning
res;
}
...
imprime <- rbsb.sii;
#imprime <- 1;
if (rbsb.mck) {check4valid(valid8bn(bn));}
...
# print(bn2dn(rbsb.bn6,10),quoi="i",simu=100);
#REFERENCE
#SEE ALSO
#CALLING {simulate8bn}
#COMMENT
...
# returning
if (rbsb.mck) { check4valid(valid8dn(dd));}
dd;
}
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
{
# checking
if (rbsb.mck) { check4valid(valid8dn(dn));}
# computing the score
sco <- dn2score(dn);
# adding the variable
res <- dn;
...
# checking
if (rbsb.mck) { check4valid(valid8dn(res));}
# returning
res;
}
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
# given by \code{rbsb.ena[2]} which are filled accordingly to the
...
if (nrow(E) == 0) {
    ...
    if (names(rbsb.ena[2]) %in% names(E)) {
      
        E[names(rbsb.ena[2])] <- DIDI;
    }
...
if (rbsb.esu) {
    ...
    form3titre("For the individual:",1);
    print(ind);
    form3titre(paste("The subset of selection (size=",length(SSS),") was:",sep=""),1);
    print(E[SSS,setdiff(names(E),names(rbsb.ena)[1])]);
    sink();
}
#
# returning
...
if (rbsb.mck) {
    ...
    check4tyle(nat,"ncharacter",c(length(wgt),Inf),"nat in draw8dist");
    if (!(expr3present(nat,rbsb.sna,how="a"))) {
