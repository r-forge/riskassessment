...
if (rbsb.mck) {
    ...
    check4tyle(quoi,rbsb.chara,1,message="'quoi' must be character(1)");
}
#
nsou <- 39;
...
if (rbsb.mck) {
    ...
    check4tyle(quoi,rbsb.chara,1,
               message="quoi (character(1)) must indicate the type of desired output");
    if (!(quoi %in% rbsb.vma)) {
...
if (rbsb.mck) {
    check4valid(valid8nom(nom));
    check4tyle(quoi,"character",1,message="'by' must be 'character(1)'");
}
## preparing
if (expr3present("n",quoi)) {
...
# empty or vide. For this last case, it
# is directed to S4 objects with boolean
# slot @vide.
#DETAILS
