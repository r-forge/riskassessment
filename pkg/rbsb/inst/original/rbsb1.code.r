...
# performed for all layers: only \code{rbsb} since it is the first one.).
# When \code{whi=="reset"} nothing (but the assignments of 
# the layer \code{rbsb} are performed).
...
sc["null"] <- "list of constant of different classes declared as NULL by 'isvide'";
...
if (length(nb)!=1) {
    erreur(nb,"lp deals only with scalar nb");
}
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
check4tyle <- function(x,typ,len,con=rbsb.num0,
...
#{con} << If length(con)>0, some check about
...
res <- c(res,comment);
res <- c(res,paste(comment,"  This character was created on",today(),"by list2file of rbsb package"));
...
if (length(lili) == 0) {
    ...
    for (ii in uv) {
        ...
        if (niv == 1) { res <- c(res,paste(comment,form3repete("=",20),sep=""));}
...
# \code{rbsb.mck}, being reported in the called functions.
#PKEYWORDS
#KEYWORDS IO
#INPUTS
