...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
rs003k <- function(whi)
#TITLE  assigns the constants for the rs00 layer
#DESCRIPTION (00)
...
#
    assign("rbsb.msi","/rbsb(0.3-0)/",pos=".GlobalEnv");
...
#{x} <<The daf object.>>
#[INPUTS]
#{quoi} << which part of the description to print
#                  (see print.ds).>>
...
if (rbsb.mck) {
    ...
    check4tyle(quoi,rbsb.chara,1,"'quoi' must be character(1)");
}
#
nsou <- 39;
...
# or a component named \code{daf} comprising a
# character(3) with des@name, what and valu slots.
#DETAILS
...
if (isempty(li$daf)) {
    ...
    # simple case
    if (rbsb.mck) {
        check4tyle(li$daf,rbsb.chara,3,"A character(3) was expected: name, what, valu");
    }
...
# or whatever
#DESCRIPTION (00) returns a scalar character of the names
...
# or whatever
#DESCRIPTION (00) returns a vector character of the names
...
#DESCRIPTION (00) returns a vector character with the 
...
#TITLE  does sch belongs to ch?
#DESCRIPTION (00)
# Checks if string(s) belong(s) to a series of strings.
...
# to the series \code{ch} and \code{'a'} when all components comply.>>
#VALUE TRUE or FALSE
#EXAMPLE
# rs003k("RESET"); # needed only for R checking, to be forgotten
# expr3present('a','non');
...
#         chain is a vector can be a vector of the same length>>
...
#KEYWORDS print
#INPUTS
#{chaine} <<The character string to frame; can be of length
#           greater than one.>>
#[INPUTS]
...
{
if (rbsb.mck) {
    check4tyle(chaine,rbsb.chara,-1,"form3encadre: 'chaine' must be a character");
    check4tyle(bef,rbsb.chara,1,"form3encadre: 'bef' must be a character(1)");
    check4tyle(aft,rbsb.chara,1,"form3encadre: 'aft' must be a character(1)");
    check4tyle(imp,"logical",1,"form3encadre: 'imp' must be a logical(1)");
    check4tyle( cr,"logical",1,"form3encadre: 'cr' must be a logical(1)");
}
if (length(chaine)>0) {
...
#TITLE  removes character before and after a character string
#DESCRIPTION (00)
# removes 'bef's before and 'aft's after a character string.
#DETAILS
#PKEYWORDS
#KEYWORDS utilities
#INPUTS
#{chaine} <<The character string to refine. 
#           Can be of length greater than one.>>
#[INPUTS]
...
# checking
if (rbsb.mck) {
    check4tyle(chaine,rbsb.chara,-1,"form3decadre: 'chaine' must be a character");
    check4tyle(bef,rbsb.chara,1,"form3decadre: Vector are not accepted for 'bef'");
    check4tyle(aft,rbsb.chara,1,"form3decadre: Vector are not accepted for 'aft'");
...
form3names <- function(nbn,nom=character(0),prefix="",
                           upca=TRUE,nume=14)
#TITLE  provides systematic names for nodes
#DESCRIPTION (00)
...
form3line <- function(len=50,pat="-_-",wid=3,
                      gind="",pat2="_-_",
                      imp=TRUE)
#TITLE  print a separator line from a given pattern
#DESCRIPTION (00)
...
#TITLE  adds a character vector into a file
#DESCRIPTION (00)
...
#DESCRIPTION (00)
# prints or prepares a character string "tit" (can comprise
...
#        options are:
#         \code{"no"} for nothing,
#         \code{"un"} for underlined,
#         \code{"ov"} for overlined,
#         \code{"par"} for parentherized,
#         \code{"cor"} for the four corners,
#         \code{"unov"} for underlined and overlined,
#         \code{"box"} for a complete box.
...
#VALUE
# either nothing but a printing is performed or
# a character string according to imp
#EXAMPLE
# rs003k("RESET"); # needed only for R checking, to be forgotten
...
#CALLING {form3repete}
#COMMENT
# Sometime the previous form3titre must be completely replaced
# by this function.
#FUTURE
#AUTHOR J.-B. Denis
#CREATED 07_09_24
...
if (is.numeric(sbef)) { sbef <- form3repete(" ",max(0,round(sbef[1])));}
if (is.numeric(saft)) { saft <- form3repete(" ",max(0,round(saft[1])));}
#
...
res <- character(0);
# first lines
for (ii in sjl(lbef)) { res <- c(res,lbef[ii]);}
# overline
...
# last lines
for (ii in sjl(laft)) { res <- c(res,laft[ii]);}
# adding carriage returns
if (caret) { for (ii in sjl(res)) {
    res[ii] <- paste(res[ii],"\n",sep="");
}}
# returning
if (imp) {
...
# This function is completely replaced by \code{form3title}
# indeed, it just some specialized calls to it. Once day, it could be
# disappeared, the user is advised to use \code{form3title}
# directly.
#DETAILS
#PKEYWORDS
#KEYWORDS print
...
if (empha == 1) { box <- "un";}
if (empha == 2) { box <- "unov";}
if (empha == 3) { box <- "unov"; lbef <- 1;}
if (empha == 4) { box <- "unov"; laft <- 1; lbef <- 1;}
if (empha == 5) { box <- "unov"; laft <- 2; lbef <- 2;}
if (empha == 6) { box <- "box" ; laft <- 1; lbef <- 1;}
# calling form3title
...
#TITLE  displays with its name the object x
#DESCRIPTION (00) everything is in the title
#DETAILS
#PKEYWORDS
#KEYWORDS print
#INPUTS
...
#DESCRIPTION (00) Transforms, into R to three, cartesian coordinates
...
#DESCRIPTION (00) (in R to the three)
#DETAILS
#PKEYWORDS
#KEYWORDS utilities
#INPUTS
...
#TITLE  transforms a numeric value into a 
# categoric value.
#DESCRIPTION (00) A first step towards the discretization
...
#TITLE  opens the graph device for rebastaba
#DESCRIPTION (00)
...
#TITLE  provides a suitable Bugs transcription for translation into 
# Bugs language
#DESCRIPTION (00)
...
#TITLE  provides the generation function from gc and ft
#DESCRIPTION (00)
...
#TITLE  returns a character giving the present day
#DESCRIPTION (00)  returns a character giving the present day
...
#DESCRIPTION (00)  returns a character giving the present moment
...
#TITLE  checks the type and the length
...
# 'Named character' are not considered as been 
# 'character'.
#PKEYWORDS
#KEYWORDS error
#INPUTS
...
# check4tyle(structure("A",.Names="a"),"ncharacter",-1,"not bad");
# check4tyle("A","ncharacter",-1,"BAD",fatal=FALSE);
# check4tyle(structure("A",.Names="a"),"character",-1,"bad",fatal=FALSE);
#REFERENCE
#SEE ALSO
#CALLING
#COMMENT
...
#[INPUTS]
#{check}<< must be checked of the consistency between xv and di
#                 be performed?>>
#VALUE
...
#TITLE  transforms a consistent list into a new des object
#DESCRIPTION (00)
...
# The components of the list must be either the
# direct transcription of the slot hierarchy of /des/
# or a component named \code{des} comprising a
# character(1:Inf) with des@name, des@orig, des@time,
# des@defi,des@role,des@comm.
#DETAILS
#PKEYWORDS des
#KEYWORDS classes
#INPUTS
...
if (isempty(li$des)) {
    ...
    # simple case
    if (rbsb.mck) {
        check4tyle(li$des,rbsb.chara,c(1,Inf),"A character(1:Inf) was expected: name,... orig, time, defi, role, comm.");
    }
    name <- li$des[1];
    nbl <- length(li$des);
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
char2des <- function(x)
#TITLE  transforms,if necessary, a single character to a 'des' object
#DESCRIPTION (00)
...
#TITLE  extrapolation of known values
#DESCRIPTION (00) From a set values given at known dates (x),
...
# checking
if (rbsb.mck) {
    check4tyle(whi,rbsb.chara,2,"A character(2) is expected");
    #
...
# checking
if (rbsb.mck) {
    check4tyle(qui,   c(rbsb.chara,"numeric"),-1,"   'qui' is not valid");
    check4tyle(dans,  c(rbsb.chara,"numeric"),-1,"  'dans' is not valid");
...
#FUTURE 
# Think about replacing '<NA>' names by no names as logical
#AUTHOR J.-B. Denis
#CREATED 08_12_10
#REVISED 10_06_23
#--------------------------------------------
...
# checking
if (rbsb.mck) {
    check4tyle(file,rbsb.chara,1,"file must indicate the name of one file");
    #
    check4tyle(path,rbsb.chara,1,"path must indicate the name of one directory");
    #
    if (path!="") { fifi <- paste(path,file,sep="/");} else { fifi <- file;}
    if (is.na(file.info(fifi)$size)) { erreur(fifi,"This file seems not to exist.");}
    #
    check4tyle(tags,rbsb.chara,c(2,Inf),"tags must be a CHARACTER matrix with two columns");
    ...
    #
    check4tyle(sep,rbsb.chara,1,"sep must indicate one string character");
    if (nchar(sep) == 0) { erreur(sep,"sep must not be an empty string");}
    #
    check4tyle(rsep,rbsb.chara,1,"rsep must be a character(1)");
    #
    check4tyle(stag,rbsb.chara,2,"stag must be a character of length two");
    ...
    #
    check4tyle(comment,rbsb.chara,1,"comment must be a character of length one");
    #
    check4tyle(end,rbsb.chara,1,"end must be a character of length one");
    #
    check4tyle(beg.skip,rbsb.chara,1,"beg.skip must be a character of length one");
    #
    check4tyle(end.skip,rbsb.chara,1,"end.skip must be a character of length one");   
}
#
...
#TITLE  transforms a list and write it onto a character file
#DESCRIPTION (00)
# The reverse operation of \code{file2list}. The list must be a
...
# it will be destroyed and recreated or completed. When it is \code{rbsb.cha0}
# then no file is written but a \code{character} is returned. For the sake of
# simple coding this is done trough a temporary file named \code{totototo} in the
# proposed path.
# >>
#[INPUTS]
#{path} << Directory containing the file.>>
...
if (rbsb.mck) {
    ...
    #
    check4tyle(file,rbsb.chara,0:1,"file must indicate the name of one file");
    #
    check4tyle(path,rbsb.chara,1,"path must indicate the name of one directory");
    #
    ...
    if (length(unique(tags[,1]))!= nrow(tags)) { erreur(tags,"Opening tags are not unique!");}
    #
    check4tyle(stag,rbsb.chara,3,"stag must be a character of length three");
    if (any(nchar(stag) == 0)) { erreur(stag,"stag must not have zero length components");}
    if (length(grep(" ",stag[c(1,3)])) > 0) { erreur(stag,"stag must not comprise spaces in first and/or third position");}
    #
    check4tyle(comment,rbsb.chara,1,"'comment' must be a character of length one");
    #
    check4tyle(comments,rbsb.chara,-1,"'comment' must be a character of any length");
}
#
...
if (length(lili) == 0) {
    ...
    for (ii in uv) {
        ...
        if (liliex[ii,"classes"] != "list") {
            ...
            cat(stag[1],stag[2],stag[3],sep="");
            cat(caca$type,"\n");
            cat(paste(caca$character,collapse=stag[2]),"\n");
        } else {
            cat("\n");
        }
    }
...
if (rbsb.mck) {
    ...
    check4tyle(tata,rbsb.chara,-1,
               "'tata' must be a 'character'");
    if (!is.matrix(tata)) {
        check4tyle(tata,rbsb.chara,length(nta),
                   "When not a matrix, 'tata' must have the same length that 'nta'");
    } else {
...
if (rbsb.mck) {
    ...
    #
    check4tyle(tata,rbsb.chara,length(nta),
               "'tata' must be a 'character' of length length(nta)");
    #
...
# \cr
# In fact \code{rbsb.vma} coding is used for the argument \code{quoi}. 
...
#[INPUTS]
#{quoi} << Indicates which structure to return: either
...
#             is done with \code{rbsb.sep0}.
#    \code{rbsb.vma["v"]} or \code{rbsb.vma["u"]} for a no named vector;
...
#    a length of 2+1+6=9.
#    \code{rbsb.vma["A"]} for a dimnamed array, the dimensions, \code{xsep}, the dimnames of each
#    dimension in the right order also separated and finished with \code{xsep}. 
#    2x3 will gives a length of 2+1+2+1+3+1+6=16.
#    \code{rbsb.vma["b"]} for a named dimensions array, the dimensions, \code{xsep}, the names for the
#    dimension in the right order not separated and finished with \code{xsep}. 
#    2x3 will gives a length of 2+1+2+1+6=12.
#    \code{rbsb.vma["B"]} for a named and dimnamed array, the dimensions, \code{xsep}, the names for the
...
# checking
if (rbsb.mck) {
    check4tyle(cha,rbsb.chara,-1,"cha must be a character");
    #
    check4tyle(quoi,rbsb.chara,1,
               "quoi (character(1)) must indicate the type of desired output");
    if (!(quoi %in% rbsb.vma)) {
        erreur(quoi,"'quoi' not in the list of possible values...");
    }
    #
    check4tyle(xsep,rbsb.chara,1,"must indicate the character string of separation");
    #
    if (quoi %in% rbsb.vma[c("V","U")]) {
        if ((length(cha) %% 2) != 0) {
            erreur(list(cha,quoi),"Here the length of 'cha' must be even");
        }
    }
    #
    if (quoi %in% rbsb.vma[c("n","N","m","M","o","O")]) {
        if (length(ssep) >= 1) {
            nbc <- ssep[1] - 1;
	...
	# an array must be returned
        ...
        if (quoi %in% rbsb.vma[c("A","B")]) { X <- X + sum(didi) + length(didi); }
        if (quoi %in% rbsb.vma[c("b","B")]) { X <- X + length(didi) + 1; }
	if (length(cha) != X) {
            erreur(list(cha,quoi,xsep),"Inconsistency for an array");
	}
    }
    #
    check4tyle(nat,rbsb.chara,1,"'nat' must be a character(1)");
}
#
res <- rbsb.cha0;
#
if (quoi %in% rbsb.vma["c"]) {
...
#
if (quoi %in% rbsb.vma[c("v","V","u","U")]) {
    # a vector must be returned
    if (quoi %in% c(rbsb.vma[c("v","u")])) {
        res <- cha;
    } else {
...
#
if (quoi %in% rbsb.vma[c("m","M","n","N","o","O","p","P")]) {
    # a matrix must be returned
    nbc <- ssep[1] - 1;
    cha <- c(cha,xsep);
    if (quoi %in% c(rbsb.vma[c("p","P")])) {
        cha <- c(" ",cha);
    }
    if (quoi %in% rbsb.vma[c("p","P")]) { add <- 1;
    } else { add <- 0; }
    cha <- matrix(cha,ncol=nbc+1+add,byrow=TRUE);
    cha <- cha[,-ncol(cha),drop=FALSE];
    if (quoi %in% rbsb.vma[c("m","M")]) {
        res <- cha;
    }
    if (quoi %in% rbsb.vma[c("n","N")]) {
        res <- cha[-1,,drop=FALSE];
        dimnames(res) <- list(NULL,cha[1,,drop=FALSE]);
    }
    if (quoi %in% rbsb.vma[c("o","O")]) {
        res <- cha[,-1,drop=FALSE];
...
if (quoi %in% rbsb.vma[c("a","A","b","B")]) {
    ...
    nbdi <- length(didi);
    #
    if (quoi == rbsb.vma["a"]) { vvv <- cha[-(1:ssep[1])];}
    if (quoi == rbsb.vma["A"]) { vvv <- cha[-(1:ssep[1+nbdi])];}
    if (quoi == rbsb.vma["b"]) { vvv <- cha[-(1:ssep[2])];}
    if (quoi == rbsb.vma["B"]) { vvv <- cha[-(1:ssep[2+nbdi])];}
    #
    res <- array(vvv,dim=didi);
    #
    if (quoi %in% rbsb.vma[c("A","B")]) {
        ndi <- vector("list",0);
        for (jj in sjl(didi)) {
            jjj <- jj + (quoi == rbsb.vma["B"]);
...
#EXAMPLE
# rs003k("reset"); # only necessary for R checking
...
if (rbsb.mck) {
    check4tyle(cha, rbsb.chara,-1," cha must be a character");
    check4tyle(redu,rbsb.chara, 1,"redu must be a character(1)");
    check4tyle(bef, rbsb.chara, 1," bef must be a character(1)");
    check4tyle(aft, rbsb.chara, 1," aft must be a character(1)");
}
if ((length(cha)>0) & (nchar(redu)>0)) {
    # removing before
    quoi <- paste(redu,bef,sep="");
    ou <- grep(quoi,cha);
    while (length(ou)>0) {
        cha <- gsub(quoi,bef,cha);
        ou <- grep(quoi,cha);
    }
    # removing before
    quoi <- paste(aft,redu,sep="");
    ou <- grep(quoi,cha);
    while (length(ou)>0) {
        cha <- gsub(quoi,aft,cha);
        ou <- grep(quoi,cha);
    }
}
# returning
...
# account the order of values of \code{x}. After that either
...
#[INPUTS]
#{ord} <<indicates the type of ordering must be done on
#        the output: "randor" for a circular random permutation;
#        "random" for a complete random permutation;
#        "order" for a sorting
#        "such" for nothing (in that case, it depends on the
