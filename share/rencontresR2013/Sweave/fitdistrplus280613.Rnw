...
\item \textbf{Diff\'erentes m\'ethodes d'estimation des param\`etres} sont propos\'ees :
  ...
  \item m\'ethode de \textcolor{blue}{minimisation d'une statistique d'ajustement (MGE)} (8 distances propos\'ees).
  \end{itemize}
...
\end{itemize}
\textcolor{blue}{N'h\'esitez pas ? nous envoyer vos retours ou id\'ees en vue de l'am\'elioration de ce package.}
\end{frame}
