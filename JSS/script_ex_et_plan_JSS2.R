...
pmixpargam <- function(q, prob, shapeP, scaleP, shapeG, rateG, ...)
	prob*pgamma(q, shapeG, rateG, ...) + (1-prob)*ppareto(q, shapeP, scaleP, ...)
dmixpargam <- function(x, prob, shapeP, scaleP, shapeG, rateG, ...)
	prob*dgamma(x, shapeG, rateG, ...) + (1-prob)*dpareto(x, shapeP, scaleP, ...)


p0 <- list(prob=1/2, shapeP=5, scaleP=5, shapeG=5, rateG=5)
danishmlemix <- fitdist(dloss, "mixpargam", start=p0, lower=c(0,rep(1e-6, 4)), upper=c(1, 50, 50, 50, 50))

pmixparlnorm <- function(q, prob, shapeP, scaleP, meanlog, sdlog, ...)
	prob*plnorm(q, meanlog, sdlog, ...) + (1-prob)*ppareto(q, shapeP, scaleP, ...)
dmixparlnorm <- function(x, prob, shapeP, scaleP, meanlog, sdlog, ...)
	prob*dlnorm(x, meanlog, sdlog, ...) + (1-prob)*dpareto(x, shapeP, scaleP, ...)

p1 <- list(prob=1/2, shapeP=5, scaleP=5, meanlog=5, sdlog=5)
danishmlemix2 <- fitdist(dloss, "mixparlnorm", start=p1, lower=c(0,rep(1e-6, 4)), upper=c(1, 50, 50, 50, 50), control=list(trace=0))

par(mar=c(4,4,2,1))
cdfcomp(list(danishmlepareto, danishmlemix, danishmlelnorm, danishmlemix2), xlogscale=TRUE, legendtext=c("Pareto", "mix Pareto-Gamma", "lognormal", "mix Pareto-lognormal"), main="different fits by MLE", lwd=2, ylim=c(.9, 1), xlim=c(5, 250))
cdfcomp(list(danishmlepareto, danishmlelnorm, danishmlemix2), xlogscale=TRUE, legendtext=c("Pareto", "lognormal", "mix Pareto-lognormal"), main="different fits by MLE", lwd=2)
cdfcomp(list(danishmlepareto, danishmlelnorm, danishmlemix2), xlogscale=TRUE, legendtext=c("Pareto", "lognormal", "mix Pareto-lognormal"), main="different fits by MLE", lwd=2, ylim=c(.9, 1), xlim=c(5, 250))
...
lapply(gofstat(fp, chisqbreaks=0:22), print)
lapply(gofstat(fnb, chisqbreaks=0:22), print)
...
cdfcomp(list(fp, fnb), legendtext=c("Poisson", "negative binomial"), discrete=TRUE)
