...
<<datendosulfan, echo=TRUE, eval=FALSE>>=
data(endosulfan)
str(endosulfan)
@
<<datendosulfan2, echo=FALSE, eval=TRUE>>=
data(endosulfan)
str(endosulfan, width = 70, strict.width = "cut")
@
...
estimations are 
certainly the most commonly used method for fitting distributions (\cite{Cullen99}). 
...
of the parametric distributions (for specified probabilities) against the empirical quantiles (\cite{Tse2009}).
...
for simple MLE fits in \cite{distrModJSS,jaloustreetal11,saketal11,kochetal12,marquetouxetal12,nordan12,
scholletal12,suuronenetal12,varoetal12,mandletal13}, for MLE fits and goodness-of-fit statistics in 
\cite{garcia12,tarnczi11,bagariaetal12,benavidesetal12,breitbach12,poduje12,Pouillot10}, for MLE fits 
