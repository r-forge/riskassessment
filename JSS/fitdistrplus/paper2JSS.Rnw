...
%%%
%%% TITLE:       fitdistrplus: an R Package for Distribution Fitting Methods
...
\author{ Marie Laure Delignette-Muller \\ Université Claude Bernard Lyon 1 \And
    Christophe Dutang \\ Université de Strasbourg}
\Plainauthor{Marie Laure Delignette-Muller}

\title{\pkg{fitdistrplus}: An \proglang{R} Package for Distribution Fitting Methods}
\Plaintitle{fitdistrplus: an R Package for Distribution Fitting Methods}
...
\Keywords{probability distribution fitting, bootstrap, censored data, maximum likelihood, moment matching, 
...
function and the histogram, which can be obtained with the \code{plotdist} function of the \pkg{fitdistrplus} package.
This function provides two plots (see Figure 1): the left-hand plot is the histogram (on a density scale) 
...
procedure (\cite{efrontibshirani94}) can be performed by using the argument \code{boot}. %to an integer above 10.
...
may be found in various packages (see the CRAN task view: Probability Distributions). 
%%%%%%%%%%% PEUT-ON METTRE UNE REF POUR LA CRAN TASK VIEW?
...
manual~\cite{fitdistrplus} for details).
% using the arguments 
...
manual~\cite{fitdistrplus} for lists of arguments that may be specific to each plot). 
...
The density plot and the CDF plot may be considered as the basic classical goodness-of-fits plots.
% ca te va mieux reformule comme ca?
...
As an example in Figure~\ref{groundbeef:comp}, none of the three fitted distributions correctly describes
...
pollutant concentration protecting 95$\%$ of the species (\cite{Posthuma2010}).
...
\begin{figure}[htb!]
  \centering
<<fitendo, echo=FALSE, fig=TRUE, width=6, height=4, eps=FALSE>>=
cdfcomp(list(fendo.ln, fendo.ll, fendo.P, fendo.B), xlogscale = TRUE,
        ylogscale = TRUE,legendtext = c("lognormal","loglogistic","Pareto","Burr"))
@
%qqcomp(list(fendo.ln,fendo.ll,fendo.P,fendo.B),xlogscale=TRUE,ylogscale=TRUE,
...
Cramer-von Mises, Kolmogorov-Smirnov and Anderson-Darling statistics \cite{Stephens86}.
...
simulation framework, especially within the package \pkg{mc2d} (\cite{mc2d}). One could refer to Pouillot and Delignette-Muller
...
minimum distance estimation method \cite{Stephens86,actuarJSS}. 
...
\begin{figure}[htb!]
  \centering
  %%% R code
<<mgegofcompplot, echo=FALSE, fig=TRUE, width=6, height=4, eps=FALSE>>=
cdfcomp(list(fendo.ln, fendo.ln.ADL, fendo.ln.AD2L),
xlogscale = TRUE, ylogscale = TRUE,
...
In this numerical experiment,  the lognormal distribution is first considered and then the Pareto type II distribution (\cite{Klugmanetal09}).
...
On the left-hand graph of Figure \ref{fig:danish:mme}, we compare the fitted distribution function between
 MME and MLE method.
The moment matching estimation provides a more cautious estimation of the insurance risk 
...
\label{Customization}
%%%%%%%%%%
Each time a numerical minimization (or maximization) is carried out in the \code{fitdistrplus} package, 
...
Right-censored data are also commonly encountered with survival data \cite{kleinmoeschberger03}.
...
\begin{figure}[htb!]
  \centering
  %%% R code
<<plotsalinity, echo=FALSE, fig=TRUE, width=6, height=4, eps=FALSE, result=hide>>=
plotdistcens(salinity,Turnbull = FALSE)
@
...
(\cite{kleinmoeschberger03,helsel05}).
...
\begin{figure}[htb!]
  \centering
  %%% R code
<<fitsalinitycdfcompplot, echo=FALSE, fig=TRUE, width=6, height=4, eps=FALSE>>=
cdfcompcens(list(fsal.ln, fsal.ll),
    legendtext=c("lognormal", "loglogistic "))
@
...
\begin{figure}[htb]
  \centering
<<fittoxocarapoisnbinomplot, echo=FALSE, results=hide, fig=TRUE, width=6, height=4, eps=FALSE>>=
cdfcomp(list(ftoxo.P,ftoxo.nb),legendtext=c("Poisson", "negative binomial"))
@
...
\bibliography{../vignettes/fitdistrplus,../vignettes/paperscitingfitdistrplus}
