...
while the kurtosis value quantifies the weight of tails in comparison to the normal distribution fro which
...
summarized or plotted (see Section~\ref{GOF:plots}). 
...
can plot any parametric distribution with specified parameter values
...
%%%%%%%%%%
\mytodo{
ADD AN INTRODUCTION ABOUT TESTS AN ARTICULATION WITH THE PREVIOUS PART 
}
...
We use a classical dataset from the Danish insurance industry published in \cite{mcneil97}.
...
serving sizes from the \code{groundbeef} dataset)}
  \label{fig:bootstrap}
\end{figure}
...
$95\%$ bootstrap confidence intervals, using the maximum likelihood
 fit of a lognormal distribution on the \code{endosulfan} dataset.
 %%% R code
