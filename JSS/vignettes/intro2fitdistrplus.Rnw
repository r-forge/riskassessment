...
\HRuleTop\\
The package \pkg{fitdistrplus} provides procedures for fitting univariate distribution
on different types of data (continuous, discreted and censored data) and allowing
different estimaiton methods (maximum likelihood, moment matching, quantile matching
and goodness-of-fit estimation).
Outputs objects of \code{fitdist} and \code{fitdistcens} functions are S3 objects, for
which kind generic methods are provided, including \code{summary}, \code{plot} and
\code{quantile}. 
Finally, this package can also handle bootstraps of parameter estimates.
Detailed examples are given in ecotoxicology and insurance contexts. 
\\
\HRuleBottom
...
While developping  the \pkg{fitdistrplus} package, our second objective  was to extend function \code{fitdistr} 
...
for optimization, 
useful in cases when classical optimization techniques not included in \code{optim} are more adequate.

In applied statistics, it is frequent to have to fit distributions to censored data. 
The \pkg{MASS} \code{fitdistr} function 
does not enable maximum likelihood estimation from this type data. Some packages 
deal with censored data, especially survival data \cite{survival}, e.g. \cite{hiranoetal94}, \cite{jordan05},
...
to continuous data, we   first use
a dataset named \code{groundbeef} which is included in our package.
This dataset contains pointwise values of
serving sizes in grams, collected in a French survey, for ground beef patties consumed by 
children under 5 years old, and is used in a quantitative risk assessment 
...
or simply by using the \code{plotdist} function.
...
@
  ...
  (serving size from the \code{groundbeef} dataset)}
  \label{plotdistcont}
\end{figure}
...
The skewness and kurtosis with their corresponding unbiased estimator of a sample 
...
(minimum, maximum, median, mean, standard deviation), skewness and Pearsons's kurtosis. By default, 
...
(see Figure~\ref{Cullenplotcont} for the \code{groundbeef} dataset). 
...
are then constructed by random sampling with replacement from that original dataset.
...
of the serving size from the \code{groundbeef} dataset and to draw the corresponding skewness-kurtosis
...
\label{FIT}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Once selected, one or more parametric distributions $f(.\vert \theta)$ may be fitted to the dataset, 
...
of the Hessian matrix at the maximum likelihood solution), (3) the correlation matrix
between parameter estimates, (4) the loglikelihood, and (5) Akaike and Schwarz information criteria 
(the so-called AIC and BIC).
Below is a call to the \code{fitdist} function to fit a Weibull distribution 
to the serving size in the \code{groundbeef} dataset.
%%% R code
...
a Weibull, a lognormal and a gamma distributions to \code{groundbeef} dataset (Figure~\ref{groundbeef:comp}).
%%% R code
...
@ 
  ...
  distributions fitted to serving sizes from the \code{groundbeef} dataset)}
...
In addition to the density plot and the CDF plot, the most classical goodness-of-fits plots, the two other plots
...
To illustrate other features of the \pkg{fitdistrplus} package, we  now use another
data set named  \code{endosulfan}, which is included in our package. This dataset contains
...
But the fit of a lognormal or a loglogistic distribution to the whole \code{endosulfan} dataset is rather bad,
...
Until here, we implicity define starting values (in the optimization process) 
for distributions defined in \proglang{R} packages
and for some few other distributions (see the help of \code{fitdist} for details).
...
c(Burr=quantile(fendo.B, probs = 0.05), empirical=quantile(ATV, probs = 0.05))
@
...
characterized by three parameters (example on the \code{endosulfan} dataset)}
  \label{fig:bootstrap}
\end{figure}
...
@
  ...
  and by MGE using  two different goodness-of-fit distances (example on the \code{endosulfan} dataset)}
  \label{plotfitMGE}
\end{figure}
...
%%% R code
<<quantilefitdist, echo=TRUE, fig=FALSE>>=
( estimate <- cbind(empirical = as.numeric(quantile(ATV, probs = 0.05)),
...
and consists in finding the value of the parameter $\theta$  that equalize the first theoretical 
...
To illustrate this method, we  use a classical dataset 
from the Danish insurance industry published in \cite{mcneil97}.
In \pkg{fitdistrplus}, the dataset is stored in \code{danishuni} and \code{danishmulti} for the univariate and
...
str(danishmulti)
@
Our first example of fitting a lognormal distribution on \code{danish} dataset uses a closed-form formula.
...
cdfcomp(list(flndanishMME, flndanishMLE), 
        legend=c("MME", "MLE"), main="Fitting a lognormal distribution",
        xlogscale=TRUE, datapch="*")
@
...
This is carried out by providing a lower and an upper bounds for the optimization process.
...
since the use may choose either (\ref{moments}) and (\ref{centmoments}).
...
cdfcomp(list(flndanishQME1, flndanishQME2, flndanishMLE), 
        legend=c("QME(1/3, 2/3)", "QME(3/4, 4/5)", "MLE"), main="Fitting a lognormal distribution",
        xlogscale=TRUE, datapch="*")
@
...
around particular quantiles, e.g. $p=99.5\%$ in the Solvency II insurance context.
...
cdfcomp(list(flndanishQME1, flndanishQME2, flndanishMLE), 
        ...
        xlogscale=TRUE, datapch=".")
@
\caption{Comparison between QME and MLE when fitting lognormal distribution on the \code{danishuni} dataset}
...
Here are examples of fits of a gamma distribution $\mathcal{G}(\alpha, \lambda)$ to 
the \code{groundbeef} dataset with various algorithms. Note that the conjugate gradient algorithm needs far more 
...
\code{fitdistcens}. But before that, it may be necessary to customize this optimization function to meet
...
@
On the \code{groundbeef} dataset, we observe that algorithms provide reasonably the same value of
...
such data is based upon maximum likelihood (see e.g. \cite{kleinmoeschberger03}). 
We chose to use this approach in our package.
...
plotdistcens(salinity)
@
  \caption{CDF plot of censored data (72-hour acute salinity tolerance of riverine macro-invertebrates from 
...
to a dataset as an S3 class object that may be easily printed,
summarized or plotted.
For the \code{salinity} dataset, a lognormal distribution or a loglogistic can be fitted 
...
This package is already used a lot by practionners and academics for simple MLE fits in \cite{kohlruckdeschel10,jaloustreetal11,saketal11,wilson12,kochetal12,marquetouxetal12,nordan12,
