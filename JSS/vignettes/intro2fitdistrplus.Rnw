...
the losse amounts collected at Copenhagen Reinsurance between 1980 and 1990.
In actuarial science, it is standard to consider positive heavy-tailed distribution and have 
a special focus on the right-tail of the distribution.
In this numerical experiment, we consider first the lognormal distribution and then the Pareto type II distribution, 
see e.g. \cite{Klugmanetal09}.
...
On the left-hand graph of Figure \ref{fig:danish:mme}, we compare the fitted distribution function between
 MME and MLE method.
We observe that the moment matching estimation provides a more cautious of the insurance risk 
...
In a second time, we choose to fit a Pareto type-II distribution, which gives more weight to 
the right-tail of the distribution.
As the lognormal distribution, the Pareto type-II has two parameters, which
...
Note that we have to pass a function (called \code{memp} in our example) 
for computing the empirical raw moment to \code{fitdist}, 
since the user may choose either Equation (\ref{moments}) or (\ref{centmoments}).
...
are far less distant (when looking at the right-tail) for the Pareto distribution than for the lognormal distribution on 
this data set. 
Furthermore for these two distributions, the MME method better fits the
right-tail of the distribution, which seems logical since empirical moments 
are influenced by large observed values.
In the previous traces, we give the values of goodness-of-fit statistics.
Whatever the statistic considered, the MLE-fitted lognormal always provides the best fit 
to the observed data.

Maximum likelihood and moment matching (the default on many softwares) estimations are certainly the most commonly 
used method for fitting distributions. But one should keep in mind that these two 
methods may give very different results. When choosing the matching moment method, the user should be aware 
...
Quantile matching estimation is performed by setting the argument \code{method}
...
As expected, the second QME fit gives more weight to the right-tail of the distribution, despite
we do not choose the Pareto type-II distribution.
...
In this paper, we present the \proglang{R} statistical package \pkg{fitdistrplus} for distribution fitting and demonstrate its use and usefulness. Our main objective while developing this package was to provide tools for helping \proglang{R} users to fit distributions to data. We have been encouraged to pursue our work by feedbacks 
from users of our package in various areas as food or environmental risk assessment, epidemiology, ecology, molecular biology, genomics, bioinformatics, hydraulics, mechanics, financial and actuarial mathematics or operations research. Indeed, this package is already used a lot by practionners and academics for simple MLE fits in \cite{distrModJSS,jaloustreetal11,saketal11,wilson12,kochetal12,marquetouxetal12,nordan12,
...
Many extensions of this package are planned in the future: 
we target to extend to censored data some methods for the moment only available for non-censored data, especially concerning goodness-of-fit evaluation and fitting methods. We will also enlarge the choice of fitting methods for non-censored data, by proposing new goodness-of-fit distances (e.g. distances based on quantiles) for maximum goodness-of-fit estimation 
...
Jean-Baptiste Denis, especially for its conceptualization.
