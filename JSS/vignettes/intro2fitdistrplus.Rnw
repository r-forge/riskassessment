...
from users of our package in various areas as food or environmental risk assessment, epidemiology, ecology, molecular biology, genomics, bioinformatics, hydraulics, mechanics, financial mathematics or operations research. Indeed, this package is already used a lot by practionners and academics for simple MLE fits in \cite{distrModJSS,jaloustreetal11,saketal11,wilson12,kochetal12,marquetouxetal12,nordan12,
...
we plan to extend to censored data some methods for the moment only available for non-censored data, especially concerning goodness-of-fit evaluation and fitting methods. We also plan enlarge the choice of fitting methods for non-censored data, by proposing new goodness-of-fit distances (e.g. distances based on quantiles) for maximum goodness-of-fit estimation 
and new types of moments (e.g. limited expected values) for moment matching estimation. At last, we plan consider multivariate distribution fitting.

%%%%%%%%%%%%%%%% redondant avec ce qui precede me semble-t-il
% et je ne comprends pas bien la notion de donnees tronquees (distributions tronquees OK mais donnees?)
