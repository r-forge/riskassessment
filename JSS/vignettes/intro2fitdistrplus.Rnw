...
the process may be developed using \proglang{R} \cite{Ricci05}. In this paper, we present our 
package \pkg{fitdistrplus} \cite{fitdistrplus} for the statistical software \proglang{R}.
Our first objective by developping
this package \pkg{fitdistrplus}
...
to help the fit of a distribution to continuous data, we  will first use
...
children under 5 years old. This data set is used in a quantitative risk assessment 
...
the modelled variable, but also by the observation of the empirical distribution.
To help the user in this observation, we developed functions to plot and
...
This function provides two plots (Figure 1): the left-hand plot is the histogram (on a density level) 
...
Together with the use of functions plotdist and \code{descdist} to characterize  the empirical distribution,
the properties of the modeled variable should be considered, especially its range, in order
to choose good candidates for the next step : fit of distributions and comparison of goodness-of-fits.

\subsection{Fit of distributions by maximum likelihood estimation}
\label{FIT}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Once selected, one or more parametric distributions $f(.\vert \theta)$ may be fitted to the dataset, one at a time, using
...
to a dataset as an S3 class object that may be easily printed, 
...
between parameter estimates, loglikelihood, and Akaike and Schwarz information criteria 
(so called AIC and BIC).
Below is a call to function \code{fitdist} to fit a Weibull distribution 
to the serving size in the ``ground beef'' dataset.
%%% R code
...
\begin{itemize}
\item a density plot representing the pdf curve of the fitted distribution 
juxtaposed with the histogram of the empirical distribution, 
\item a cumulative distribution function (cdf) plot 
of both the empirical distribution and the fitted distribution, 
\item a Q-Q plot
representing the empirical quantiles (y-axis) against the quantiles of the theoretical fitted distribution (x-axis)
\item and a P-P plot representing the empirical distribution function evaluated at each data point (y-axis)
against the fitted distribution function (x-axis).
\end{itemize}

Functions \code{denscomp}, \code{cdfcomp},
\code{qqcomp} and \code{ppcomp}, enable to separately plot each
of these four plots, in order to compare the empirical and various theoretical distributions fitted
on a same dataset. These functions must be called with a first argument 
...
Weibull, lognormal and gamma distributions to \code{groundbeef} dataset (Figure~\ref{groundbeef:comp}).
%%% R code
...
@ 
  ...
  distributions fitted to serving sizes from the ``groundbeef'' dataset)}
\label{groundbeef:comp}  
\end{figure}


For cdf, Q-Q and P-P plots, the probability plotting position is defined as recommended by Blom \cite{Blom},
by default using the Hazen's rule, with probability points of the empirical distribution defined as
 \code{(1:n - 0.5)/n}. This plotting
position can be easily changed using arguments \code{use.ppoints} and \code{a.ppoints}. When \code{use.ppoints = TRUE}, 
the argument \code{a.ppoints} is passed to Function \code{ppoints} from the \pkg{stats} package to define the 
...
If the density plot and the cdf plot are the most classical goodness-of-fits plots, the two other plots
...
if the weight of this tail is important in the use of the fitted distribution.


To illustrate other features of package \pkg{fitdistrplus}, we will now use another
data set named  ``endosulfan'', which is included in our package. This data set contains
...
%is used to estimate the hazardous concentration affecting $x\%$ of the species ($HCx$), generally with $x$ fixed
%at 5 ($HC5$). 
It is interpreted as the value of the
pollutant concentration protecting 95$\%$ of the species.
But the fit of a lognormal or a loglogistic distribution to the whole ``endosulfan'' data set is rather bad,
...
are provided in package \pkg{actuar}. For distributions defined in \proglang{R} packages
and for some few other distributions (see the help of \code{fitdist} for details), it is necessary to specify initial 
values for the distribution parameters in the argument \code{start} when using the maximum likelihood method. \code{start} must be a named list
of parameters initial values. 
%The names of
%the parameters in \code{start} must correspond exactly to their definition in \proglang{R} or 
%in a user-supplied \proglang{R} code.
Function \code{plotdist} can plot any parametric distribution with specified parameter values
...
Having defined reasonable starting values, we can fit various distributions and graphically
...
describes the right tail observed in the data set, but that the left tail seems to 
be better described by the Burr distribution. Its use could then be considered
to estimate the $HC5$ value as the $5\%$ quantile of the distribution. This can be 
easily done using the generic Function quantile defined for an object 
of class \code{"fitdist"}. 
%%% R code
...
%(also named the extreme value distribution) on the \code{groundbeef} dataset.
...
%  distribution fitted to serving sizes from the \code{groundbeef} dataset)}
%  \label{plotcontfit}
%\end{figure}
...
%  (Weibull, gamma and lognormal
%  distributions fitted to serving sizes from the ``ground beef'' dataset)}
%  \label{compdens}
%\end{figure}
%
%\begin{figure}[htb]
...
fitted parametric distribution and the empirical distribution.
Such a distance can be measured beetween
...
%%% R code
%<<fendo.gof.echo, echo=TRUE, results = hide,fig=FALSE>>=
%gofstatres <- cbind(
...
as it is often the case in risk assessment \cite{Cullen99,Vose10}. For this reason, 
...
cdf quadratic difference is dependent on the theoretical distribution in its definition. 
Anderson-Darling statistics computed for several distributions fitted on the same data set
...
bootstrap using the \code{boodist} function for non censored data.
...
Below is an example of the use of Function \code{bootdist} with the previous fit of the Burr distribution to the
\code{endosulfan} dataset. 
%%% R code
...
%Then we fit the three-parameter distribution of Burr on \code{danishuni} dataset.
...
characterized by three parameters (example on the fit of a Burr distribution to
acute toxicity values from the \code{endosulfan} dataset)}
  \label{fig:bootstrap}
\end{figure}
...
to look at the potential structural correlation between parameters (see Figure~\ref{fig:bootstrap}).
...
Burr distribution to the \code{endosulfan} dataset.
 %%% R code
...
%Below an example of estimation on the \code{danishuni} dataset with the three classical 
...
In the following example of \code{endosulfan} dataset, we use left tail 
...
xlegend = "bottomright")
@
  \caption{Comparison of one distribution fitted by maximum likelihood estimation
  and by maximum goodness-of-fit using
  two different goodness-of-fit distances (example of a lognormal distribution fitted to acute toxicity values
  from the \code{endosulfan} dataset)}
  \label{plotfitMGE}
\end{figure}
...
%%% R code
<<quantilefitdist, echo=TRUE, fig=FALSE>>=
cbind(quantile(fendo.B,probs = 0.05)$quantiles,
      quantile(fendo.ln,probs = 0.05)$quantiles,
      quantile(fendo.ln.ADL,probs = 0.05)$quantiles,
      quantile(fendo.ln.AD2L,probs = 0.05)$quantiles)
@
...
%%%%%%%%%%
\mytodo{
Christophe, I let you change from here to  the end of section 3.2  
}
...
To illustrate this method, we will use a classical dataset from the Danish insurance industry published in \cite{mcneil97}.
In \pkg{fitdistrplus}, the dataset is stored in \code{danishuni} and \code{danishmulti} for univariate and
...
Our first example of fitting a lognormal distribution on \code{danish} dataset uses a closed formula.
...
cdfcomp(list(flndanishQME1, flndanishQME2, flndanishMLE), 
        ...
        xlogscale=TRUE, datapch="*")
@
Above is an example of fitting of a lognormal distribution to \code{danishuni} dataset 
...
Here are examples of fits of a gamma distribution to \code{groundbeef} dataset with various options
...
\code{groundbeef} dataset. Note that in this example various arguments are also passed from \code{fitdist} 
to \code{genoud} :
...
such data is based upon maximum likelihood (!!! ADD A REFERENCE : HELSEL BOOK OR ANOTHER ONE
IF POSSIBLE WHO GIVES THE FORMULA !!!). We chose to use this approach in our package.
...
acute salinity tolerance (LC50 values in electrical conductivity, $mS$.$cm^{-1}$)
of riverine macro-invertebrates taxa from the southern Murray-Darling Basin in 
Central Victoria, Australia
(!!!! ADD Kefford06 in the bibtex and cite it here !!!!!!!!!). 

%The \code{smokedfish} dataset, included in the package,
...
Using censored data such as those coded in the \code{salinity} dataset,
...
Let us see such a plot for \code{salinity} dataset(Figure~\ref{plotdistcens}).
...
plotdistcens(salinity)
@
  \caption{CDF plot of censored data (72-hour acute salinity tolerance (LC50 values) of riverine macro-invertebrates from 
   the \code{salinity} dataset)}
  \label{plotdistcens}
\end{figure}
...
be fitted to the censored dataset, one at a time, but using in this case
Function \code{fitdistcens}. This function estimates the vector of distribution parameters $\theta$
...
to a dataset as an S3 class object that may be easily printed,
summarized or plotted.
For ``salinity'' dataset, a lognormal distribution or a loglogistic can be fitted 
...
fsal.ll <- fitdistcens(salinity,"llogis",start=list(shape=5,scale=30))
summary(fsal.ln)
summary(fsal.ll)
@
...
respectively provided when summarizing or printing an object of class \code{"fitdistcens"}. 
Function \code{cdfcompcens} can also be used to compare the fit of various
distributions to the same censored dataset. Its call is similar to the one of \code{cdfcomp}. Below is
an example of comparison of the two fitted distributions to the \code{salinity} dataset (see Figure~\ref{cdfcompcens}).
...
cdfcompcens(list(fsal.ln, fsal.ll),
    legendtext=c("lognormal", "loglogistic "))
@
  \caption{Goodness-of-fit CDF plot for fits of continuous distributions
  to censored data (Comparison of lognormal and loglogistic distributions fitted to LC50 values from 
   the \code{salinity} dataset)}
  \label{cdfcompcens}
\end{figure}
...
%   the ``smoked fish'' dataset)}
%  \label{plotfitdistcens}
%\end{figure}


Un addition to the fit of distributions to censored or non censored continuous data, our package
can also accomodate discrete variables, such as count numbers. Specific distributions such as 
the Poisson distribution of the negative binomial distribution can be fitted with the same functions
used for continuous non-censored data. These functions will provide somewhat different graphs
...
from a random sampling of feral 
...
In some cases a discrete variable may be plotted as a continuous one, for example for a large dataset
...
@
  ...
  (number of \emph{Toxocara cati} parasites from the \code{toxocara} dataset)}
  \label{plotdistdisc}
\end{figure}
...
%the ``Toxacara'' dataset, one could try the fit of Poisson and negative-binomial distributions.
...
%  (number of \emph{Toxocara cati} parasites from the ``Toxocara'' dataset)}
...
As an example, 
using the \code{toxocara} dataset, Poisson and
...
in pdf and in cdf. Function \code{cdfcomp} can also be used to compare several plots
to the same data set, as follows for the previous fits. 
...
@
  ...
  fitted to numbers of \emph{Toxocara cati} parasites from the \code{toxocara} dataset)}
  \label{plotdiscfit}
\end{figure}
...
the fits of the Poisson and the negative binomial distribution to \code{toxocara} dataset 
