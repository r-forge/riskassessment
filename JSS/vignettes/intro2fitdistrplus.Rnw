...
of the fitted distribution, generally the 5$\%$ percentile, named the hazardous concentration 5$\%$ (HC5)
%is used to estimate the hazardous concentration affecting $x\%$ of the species ($HCx$), generally with $x$ fixed
%at 5 ($HC5$). 
This value is interpreted as the value of the
...
\mytodo{
REPLACE BY A CALL TO PRINT OF NEW FUNCTION gofstat 
}
...
cdf quadratic difference is dependent of the theoretical distribution in its definition. 
Anderson-Darling statistics computed for several distributions fitted on the same data set
are thus theoretically difficult to compare. Moreover, such a statistics, as Cramer-von Mises and
...
the Burr function, the only one characterized by three parameters, while Akaike and Schwarz information criteria 
...
Going back to he previous axample from ecotoxicolgy, this function can be used to 
...
consider the fit of the classical lognormal distribution, but maximizing a
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\mytodo{
WRITE AN INTRODUCTION, change the example for censored data, and revise all this part  .... 
}
...
Using censored data such as those coded in the \code{smokedfish} dataset,
...
Let us see such a plot for \code{smokedfish} dataset after classical transformation
...
to a dataset as an S3 class object that may be easily printed,
summarized or plotted.
For ``smokedfish'' dataset, a normal distribution may be fitted to log
...
distributions to the same censored dataset. Its call is similar to the one \code{cdfcomp}. Below is
an example of comparison of two fitted distribution to \code{smokedfish} dataset (see Figure~\ref{cdfcompcens}).
%%% R code
...
The \code{toxocara} dataset corresponds to the observation of a discrete variable, 
...
the fit of a negative binomial distribution to \code{toxocara} dataset gives following results :
%%% R code
