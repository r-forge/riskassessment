...
In the \proglang{R} (\cite{R13}) package \pkg{MASS} (\cite{MASS}), maximum likelihood estimation 
is available via the \code{fitdistr} function; then other steps of the
fitting process can be done using other \proglang{R} functions, e.g. \cite{Ricci05}. 
In this paper, we present the \proglang{R} package \pkg{fitdistrplus} (\cite{fitdistrplus})
implementing several methods of fitting univariate parametric distribution.
A first objective by developing this package was to provide \proglang{R} 
...
such as maximum goodness-of-fit estimation (also called minimum distance estimation), 
...
for examples),
but those packages generally focused on specific models, enabling the fit of  a 
restricted set of distributions. 
A third objective was thus to provide \proglang{R} users a function 
to estimate univariate distribution parameters from censored data 
both for left/right-censoring and interval-censoring. 
...
implementation of probability models for non-censored data
and includes distribution fitting procedures for a given minimization criterion.
This package is based on the \proglang{R}-shipped function \code{mle} of the \pkg{stats4} package
producing S4 objects of class \code{"mle"} for which generic methods are 
implemented, e.g. \code{confint} and \code{logLik}.
In \pkg{fitdistrplus}, we choose to use the standard S3 class system for its understanding by most \proglang{R} users.
When designing the \pkg{fitdistrplus} package, we do not forget to implement
...
published in the International Journal of Food Microbiology \cite{Delignette08}.
...
the modelled variable and by the observation of its empirical distribution.
...
function and the histogram  using the \code{plotdist} function of the \pkg{fitdistrplus} package.
This function provides two plots (see Figure 1): the left-hand plot is the histogram (on a densityf) 
...
The skewness and kurtosis and their corresponding unbiased estimator (e.g. \cite{casellaberger02}) from a sample 
...
unbiased estimations of the three last statistics are provided (the argument \code{method}
allows to obtain  them without correction for bias). 
...
of the estimated values of kurtosis and skewness from data, a bootstrap 
procedure (\cite{efrontibshirani94}) can be performed by fixing the argument \code{boot}. %to an integer above 10.
...
The properties of the random variable should be considered, notably its expected value, its range,
as a complement to the use of the \code{plotdist} and \code{descdist}. 
...
for which a \code{print}, a \code{summarize} and a \code{plot} methods are provided.
In order to be used in \code{fitdist}, a distribution  has to be specified via
the argument \code{dist} either a character string (its classical abbreviation) 
or a function (from which the root name is extracted).
The \code{fitdist} function assumes that the corresponding 
\code{d}, \code{p}, \code{q} functions (standing respectively for the density, 
the cumulative distribution and the quantile functions) are defined.
This is automatic for classical distributions of the \pkg{stats} package, see \code{?Distributions},
e.g. \code{dnorm}, \code{pnorm} and \code{qnorm} for the normal distribution,
otherwise the user has to implement them.
...
presented on Figures \ref{groundbeef:comp} and \ref{endo:comp}: 
\begin{itemize}
...
\code{qqcomp} and \code{ppcomp} functions enable to separately get each
of these four plots, in order to compare the empirical and multiple parametric distributions fitted
...
manual~\cite{fitdistrplus} for lists of arguments). 
...
ppcomp(list(fw,fln,fg), legendtext=c("Weibull", "lognormal", "gamma"))
@ 
  \caption{Two Goodness-of-fit plots for various distributions fitted on continuous data
...
%%%%%%
% je pense qu'il faudrait mettre les deux paragraphes suivants plus haut? apres le itemize
...
The density plot and the CDF plot are the most classical goodness-of-fits plots. % je ne suis pas trop de son avis, mais on peut peut etre reformules?
...
The data set named  \code{endosulfan} is now used to illustrate other features of 
...
of the fitted distribution, generally the 5$\%$ percentile, is calculated and named 
...
Pareto and Burr distributions are provided in package \pkg{actuar}. 
Until here, we did not have to  define starting values (in the optimization process)
as reasonable starting values are implicity defined within function \code{fitdist}
...
compared. On this example, the use of functions \code{cdfcomp} and
...
As shown in Figure~\ref{endo:comp}, none of the fitted distribution correctly
...
the Burr distribution, the only one characterized by three parameters, while Akaike and Schwarz's Bayesian information criteria 
...
The uncertainty in the parameters of the fitted distribution may be estimated by parametric or nonparametric
bootstraps using the \code{boodist} function for non-censored data, see \cite{efrontibshirani94}.
This function returns the bootstrapped values of parameters in a 
S3 class object which may be plotted to visualize the bootstrap region. 
The medians and the 95 percent confidence intervals of parameters (2.5 and 97.5 percentiles) are printed 
in the summary.
When inferior to the whole number of iterations, the number of iterations for which the estimation converges 
...
minimum distance estimation method, e.g. \cite{Stephens86}. In this package this method is proposed with
...
Going back to the previous example from ecotoxicology, 
...
geometric, beta and uniform distributions (i.e. distributions already implemented in \proglang{R}).
...
published in \cite{mcneil97} is used to illustrate this method.
...
In this numerical experiment,  the lognormal distribution is first considered and then the Pareto type II distribution, 
see e.g. \cite{Klugmanetal09}.

The lognormal distribution on \code{danishuni} data set is 
fitted by matching moments implemented as a closed-form formula.
On the left-hand graph of Figure \ref{fig:danish:mme}, we compare the fitted distribution function between
 MME and MLE method.
The moment matching estimation provides a more cautious of the insurance risk 
...
In a second time, a Pareto type-II distribution, which gives more weight to 
...
Note that a function (called \code{memp} in our example) 
for computing the empirical raw moment to \code{fitdist} has to be passed, 
...
As shown on Figure \ref{fig:danish:mme}, matching moments and maximum likelihood fits
...
Maximum likelihood and moment matching (the default on many softwares) estimations are 
certainly the most commonly used method for fitting distributions. 
...
when choosing the matching moment estimation (MME). This may be seen as an advantage in our example if the
...
\label{Customization}
%%%%%%%%%%
Each time a numerical minimization (or maximization) is carried out in \code{fitdist}, 
...
It is also possible to use another function than \code{optim} to maximize the likelihood by
specifying by the argument \code{custom.optim} in the call to \code{fitdist}. 
But, it may be necessary to customize this optimization function to meet
...
Such data must be coded into a dataframe with two columns, respectively named \code{left} 
...
and F the cumulative distribution function of the parametric distribution, e.g. \cite{kleinmoeschberger03}.

As \code{fitdist}, \code{fitdistcens} returns the results of the fit of any parametric distribution
to a data set as a S3 class object that can be easily printed,
...
To our knowledge, computations of goodness-of-fit statistics have not yet been 
...
The \proglang{R}  package \pkg{fitdistrplus} allows us to easily fit distributions. 
...
or operations research. Indeed, this package is already used a lot by practionners and academics 
for simple MLE fits in \cite{distrModJSS,jaloustreetal11,saketal11,wilson12,kochetal12,marquetouxetal12,nordan12,
scholletal12,suuronenetal12,varoetal12,mandletal13}, for MLE fits and goodness-of-fit statistics in 
\cite{garcia12,tarnczi11,bagariaetal12,benavidesetal12,breitbach12,poduje12,pouillotetal10}, for MLE fits 
and bootstrap in  \cite{croucheretal12,meheustetal12,orellanoetal12,telloetal12,hoelzeretal12}, for MLE fits, 
bootstrap and goodness-of-fit statistics in \cite{rebuge12}, for MME fit in \cite{luangkesornetal12}, for 
censored MLE and bootstrap in \cite{lehaetal11,poulliotetal12,jongenburgeretal12,commeauetal12,pouillotetal11}, 
for graphic analysing in \cite{anandetal12,rosa12}, for grouped-data fitting methods in  \cite{fusteinercostafreda12} 
and more generally in  \cite{eling12,busschaertetal10,brooksetal11,aktassjostrand11,eikhermann12,bakos11}.
